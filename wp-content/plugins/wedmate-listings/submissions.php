...
function wml_submission_fields() {
    return [
        ...
        'portfolio_url'=>['Portfolio or photo-gallery links (optional)','textarea',false,10000],
...
function wml_portfolio_links($value) {
    $links = [];
    foreach (preg_split('/[,\r\n]+/', $value) as $link) {
        $link = trim($link);
        if ($link === '') continue;
        if (!filter_var($link, FILTER_VALIDATE_URL) || !in_array(strtolower((string)wp_parse_url($link, PHP_URL_SCHEME)), ['http', 'https'], true)) {
            return new WP_Error('portfolio_url', 'Each portfolio link must be a full http:// or https:// URL. Separate links with commas.');
        }
        $links[] = esc_url_raw($link, ['http', 'https']);
    }
...
function wml_accept_submission($raw) {
    $errors = new WP_Error();
    ...
    $links = wml_portfolio_links($scalar('portfolio_url'));
    if (is_wp_error($links)) $errors->add('portfolio_url', $links->get_error_message());
    else $data['portfolio_url'] = implode("\n", $links);
    ...
    try {
        ...
        delete_option($lock);
        return new WP_Error('save','Your listing was not submitted. Please try again.');
    } finally {
        $_POST = $original_post;
    }
}
...
add_action('template_redirect', function() {
    ...
    if (!$data && (int)($_SERVER['CONTENT_LENGTH'] ?? 0) > wp_convert_hr_to_bytes(ini_get('post_max_size'))) {
        $GLOBALS['wml_submission_errors'] = new WP_Error('size','The submission is too large. Please shorten your details and submit again.'); return;
    }
    $result = wml_accept_submission($data);
...
function wml_owner_review_box($post) {
    ...
    if ($portfolio) {
        $links = wml_portfolio_links($portfolio);
        echo '<h4>Submitted portfolio / gallery links</h4>';
        if (is_wp_error($links)) echo '<p>'.esc_html($portfolio).'</p>';
        else {
            echo '<ul>';
            foreach ($links as $link) echo '<li><a href="'.esc_url($link).'" target="_blank" rel="noopener noreferrer">'.esc_html($link).'</a></li>';
            echo '</ul>';
        }
    }
