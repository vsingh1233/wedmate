...
<div class="wml-form-wrap">
    ...
    <?php if ($received): ?>
    <section class="wml-form-success" role="status"><p class="wml-eyebrow">THANK YOU</p><h1>Your listing has been submitted</h1><p>Our team will review your details and portfolio links before publishing. Your business is not listed publicly yet.</p><p>If we need more information, we’ll use the private contact details you provided.</p><a class="wml-submit-button" href="<?php echo esc_url(wml_directory_url()); ?>">Explore Wedmate</a></section>
    <?php else: ?>
    ...
    <?php if ($errors): ?><div class="wml-form-errors" role="alert" tabindex="-1"><h2>Please check your submission</h2><ul><?php foreach ($errors->get_error_messages() as $message): ?><li><?php echo esc_html($message); ?></li><?php endforeach; ?></ul></div><?php endif; ?>
    <form class="wml-owner-form" method="post" action="<?php echo esc_url(wml_submission_url()); ?>">
        ...
        <fieldset><legend>Photos and portfolio</legend><p>Share links to your photos, galleries or portfolio, separated by commas. Include https:// or http:// for each link and make sure our review team can open them.</p><div class="wml-form-grid"><?php wml_form_field('portfolio_url',$values); ?></div></fieldset>
