...
class ET_Builder_Module_Order {
	...
	/**
	 * Capture the current module order index state for later restoration.
	 *
	 ...
	 * `restore_state()` with the returned snapshot to undo any index mutations
	 * introduced by the nested call. The snapshot is a deep copy so later
	 * writes to `$_indices` cannot mutate it.
	 *
	 * @return array Snapshot of the current module order indices.
	 * @since 5.13
	 */
	public static function capture_state(): array {
		$encoded = wp_json_encode( self::$_indices );
		$copied  = is_string( $encoded ) ? json_decode( $encoded, true ) : [];

		return is_array( $copied ) ? $copied : [];
	}

	/**
	 * Restore the module order index state from a previously captured snapshot.
	 *
	 * Reverses any index mutations introduced by a nested `do_blocks()` call so
