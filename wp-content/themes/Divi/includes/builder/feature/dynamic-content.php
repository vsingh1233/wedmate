...
function et_builder_sanitize_dynamic_content_fields( $data ) {
	...
	// if there are dynamic content items, force enable_html off.
	...
	// Normalize Builder 5 $variable tokens with the same helper used on the D5 save path.
	if ( class_exists( 'ET\Builder\Security\DynamicContent\DynamicContentFixes' ) ) {
		$data = \ET\Builder\Security\DynamicContent\DynamicContentFixes::disable_html_tokens( $data );
	}
