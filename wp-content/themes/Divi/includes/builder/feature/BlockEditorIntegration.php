...
if ( ! class_exists( 'ET_Builder_Block_Editor_Integration' ) ) {
	...
	class ET_Builder_Block_Editor_Integration {
		...
		/**
		 * Resolve the current block editor post type.
		 *
		 * @since 5.13
		 *
		 * @return string
		 */
		protected function _get_block_editor_post_type() {
			global $typenow, $post, $pagenow;

			$post_type = '';

			// phpcs:disable WordPress.Security.NonceVerification -- Reading GET for post type context only.
			if ( ! empty( $typenow ) ) {
				$post_type = $typenow;
		...
		/**
		 * Whether Divi block editor integration assets should load for the current screen.
		 *
		 * @since 5.13
		 *
		 * @return bool
		 */
		protected function _should_enqueue_block_editor_integration() {
			$post_type = $this->_get_block_editor_post_type();
			...
			if ( empty( $post_type ) ) {
				// Ambiguous post editor context; preserve legacy load behavior for core editors.
				return true;
			}
		...
		private function _register_block_editor_integration_assets() {
			// ETlogos @font-face for Divi placeholder icon in Editor Canvas.
			if ( ! wp_style_is( 'library-menu-styles', 'registered' ) ) {
				wp_register_style(
					'library-menu-styles',
					ET_BUILDER_URI . '/styles/library_menu.css',
					array(),
					ET_BUILDER_VERSION
				);
			}
		...
		public function enqueue_block_editor_assets() {
			if ( ! $this->_should_enqueue_block_editor_integration() ) {
				return;
			}
