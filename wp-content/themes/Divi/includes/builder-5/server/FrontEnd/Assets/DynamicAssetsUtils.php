...
class DynamicAssetsUtils {
	...
	public static function extract_used_fonts_from_content( $post_id ) {
		...
		$content = self::get_renderable_content_for_font_extraction( $content );
	...
	/**
	 * Filter serialized block content down to blocks that can render on the frontend.
	 *
	 * This removes Conditional Display-false subtrees and fully disabled modules from
	 * Dynamic Assets font extraction so hidden content does not enqueue visible webfonts.
	 *
	 * @since ??
	 *
	 * @param string $content Raw serialized block content.
	 *
	 * @return string Serialized content limited to renderable blocks.
	 */
	private static function get_renderable_content_for_font_extraction( string $content ): string {
		...
		$filtered_blocks = self::filter_renderable_blocks_for_font_extraction( $parsed_blocks );

		return empty( $filtered_blocks ) ? '' : serialize_blocks( $filtered_blocks );
	}

	/**
	 * Recursively remove blocks that will not render on the frontend.
	 *
	 * @since ??
	 *
	 * @param array<int, array<string, mixed>> $blocks Parsed blocks.
	 *
	 * @return array<int, array<string, mixed>> Filtered parsed blocks.
	 */
	private static function filter_renderable_blocks_for_font_extraction( array $blocks ): array {
		$filtered_blocks = [];

		foreach ( $blocks as $block ) {
			$filtered_block = is_array( $block )
				? self::filter_renderable_block_for_font_extraction( $block )
				: null;

			if ( null !== $filtered_block ) {
	...
	/**
	 * Filter one parsed block and keep `innerContent` aligned with kept children.
	 *
	 * @since ??
	 *
	 * @param array<string, mixed> $block Parsed block.
	 *
	 * @return array<string, mixed>|null Null when the block should be skipped.
	 */
	private static function filter_renderable_block_for_font_extraction( array $block ): ?array {
		$block_name = $block['blockName'] ?? '';

		if ( is_string( $block_name ) && 0 === strpos( $block_name, 'divi/' ) ) {
			$raw_attrs = is_array( $block['attrs'] ?? null ) ? $block['attrs'] : [];

			// Match early FE suppress: resolve preset renderAttrs before gating so preset-only
			// Conditional Display / full disabledOn do not leak webfonts from hidden content.
			$attrs = ModuleRegistration::resolve_module_attrs_for_renderability( $block_name, $raw_attrs );

			if ( ! ConditionsRenderer::is_displayable_attrs( $attrs ) ) {
				return null;
			}
		...
		$original_inner_blocks = is_array( $block['innerBlocks'] ?? null ) ? $block['innerBlocks'] : [];

		if ( is_array( $block['innerContent'] ?? null ) ) {
			$filtered_inner_blocks   = [];
			$filtered_inner_content  = [];
			$original_child_position = 0;

			foreach ( $block['innerContent'] as $inner_content_chunk ) {
				if ( null !== $inner_content_chunk ) {
					$filtered_inner_content[] = $inner_content_chunk;
					continue;
				}

				$child_block = $original_inner_blocks[ $original_child_position ] ?? null;
				...
				$filtered_child_block = self::filter_renderable_block_for_font_extraction( $child_block );

				if ( null !== $filtered_child_block ) {
					$filtered_inner_blocks[]  = $filtered_child_block;
					$filtered_inner_content[] = null;
				}
			}

			$block['innerBlocks']  = $filtered_inner_blocks;
			$block['innerContent'] = $filtered_inner_content;
		} elseif ( ! empty( $original_inner_blocks ) ) {
			$block['innerBlocks'] = self::filter_renderable_blocks_for_font_extraction( $original_inner_blocks );
		}
	 ...
	 *
	 * Header/footer-only templates may omit a layout key. Skip non-array slots
	 * before reading override/enabled/id so PHP 8 does not warn on null offsets.
	 *
	 * @since ??
	 *
	 * @return array
	...
	public static function get_theme_builder_template_ids(): array {
		...
		foreach ( $layout_post_types as $layout_post_type ) {
			$layout = $tb_layouts[ $layout_post_type ] ?? null;

			// Match et_theme_builder_get_template_layouts(): never index a non-array slot.
			if ( ! is_array( $layout ) || empty( $layout['override'] ) ) {
				continue;
			}

			// Include if enabled OR if on special page with override=true.
			if ( ! empty( $layout['enabled'] ) || $is_special_page ) {
				$template_id = isset( $layout['id'] ) ? (int) $layout['id'] : 0;
	...
	/**
	 * Drop cached canvas metadata whose backing `et_pb_canvas` post no longer exists.
	 *
	 * Applied on cache hit so a stale Theme Builder layout cache cannot inject a deleted canvas.
	 * Entries without a `postId` are kept because existence cannot be proven.
	 * The pruned result is not written back to post meta.
	 *
	 * @since ??
	 *
	 * @param array $all_canvas_metadata Cached canvas metadata keyed by canvas ID.
	 *
	 * @return array Filtered canvas metadata.
	 */
	private static function _filter_missing_canvas_posts_from_metadata( array $all_canvas_metadata ): array {
		if ( empty( $all_canvas_metadata ) ) {
			return $all_canvas_metadata;
		}

		$canvas_post_ids = [];
		foreach ( $all_canvas_metadata as $canvas_meta ) {
			if ( ! is_array( $canvas_meta ) ) {
				continue;
			}

			$canvas_post_id = absint( $canvas_meta['postId'] ?? 0 );
			if ( 0 < $canvas_post_id ) {
				$canvas_post_ids[] = $canvas_post_id;
			}
		}

		$canvas_post_ids = array_values( array_unique( $canvas_post_ids ) );
		if ( empty( $canvas_post_ids ) ) {
			return $all_canvas_metadata;
		}

		$existing_post_ids = array_map(
			'absint',
			get_posts(
				[
					'post_type'              => OffCanvasHooks::GLOBAL_CANVAS_POST_TYPE,
					'post_status'            => 'any',
					'post__in'               => $canvas_post_ids,
					'posts_per_page'         => count( $canvas_post_ids ),
					'fields'                 => 'ids',
					'no_found_rows'          => true,
		...
		$existing_lookup   = array_fill_keys( $existing_post_ids, true );

		$filtered_metadata = [];
		foreach ( $all_canvas_metadata as $canvas_id => $canvas_meta ) {
			if ( ! is_array( $canvas_meta ) ) {
				$filtered_metadata[ $canvas_id ] = $canvas_meta;
				continue;
			}
			...
			if ( isset( $existing_lookup[ $canvas_post_id ] ) ) {
				$filtered_metadata[ $canvas_id ] = $canvas_meta;
			}
		}
	...
	public static function get_all_canvas_data_for_post( $post_id, $main_content = '' ) {
		...
		if ( is_array( $cached_data ) && isset( $cached_data['all_canvas_metadata'] ) && is_array( $cached_data['all_canvas_metadata'] ) ) {
			...
			// Drop cached entries whose et_pb_canvas post no longer exists.
			// Do not persist the pruned result; frontend GET must remain read-only.
			$unprocessed_cached_data['all_canvas_metadata'] = self::_filter_missing_canvas_posts_from_metadata(
				$unprocessed_cached_data['all_canvas_metadata']
			);
