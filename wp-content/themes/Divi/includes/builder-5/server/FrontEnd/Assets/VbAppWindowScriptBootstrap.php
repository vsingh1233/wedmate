...
class VbAppWindowScriptBootstrap {
	...
	public static function wire_combined_dependencies(): void {
		...
		// Clear stale D4 google-maps-api registration when Theme Option disables enqueue.
		if ( function_exists( 'et_pb_enqueue_google_maps_script' ) && ! et_pb_enqueue_google_maps_script() ) {
			wp_dequeue_script( 'google-maps-api' );
			wp_deregister_script( 'google-maps-api' );
		}

		foreach ( self::_get_combined_dependency_handles() as $dep ) {
			if ( 'google-maps-api' === $dep ) {
				if ( ! function_exists( 'et_pb_enqueue_google_maps_script' ) || ! et_pb_enqueue_google_maps_script() ) {
					continue;
				}
			}
