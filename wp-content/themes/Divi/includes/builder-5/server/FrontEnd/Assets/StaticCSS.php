...
class StaticCSS implements DependencyInterface {
	...
	public static function setup_styles_manager( int $post_id = 0 ) {
		...
		// Check the resource owner and active render context during Theme Builder layout rendering.
		// The active post is the layout while the resource owner is the main post.
		$owner_has_paginated_loops = 0 < $post_id && LoopUtils::has_paginated_loops( $post_id );
		$has_paginated_loops       = $owner_has_paginated_loops || LoopUtils::has_paginated_loops();

		// Disable static CSS when either context has random-order loops to prevent stale CSS caching.
		$owner_has_random_order_loops        = 0 < $post_id && LoopUtils::current_page_has_random_order_loops( $post_id );
		$current_page_has_random_order_loops = $owner_has_random_order_loops || LoopUtils::current_page_has_random_order_loops();
