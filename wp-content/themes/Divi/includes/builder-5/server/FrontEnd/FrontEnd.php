...
class FrontEnd {
	...
	public function initialize(): void {
		...
		// The Divi theme chooses its default-header menu script on `wp_enqueue_scripts`
		// at priority 10, so priority 100 sees a final theme queue. Run this before
		// `override_d4_fe_scripts()`, which wires the enqueued Menu runtime as a
		// dependency of the combined runtime.
		add_action( 'wp_enqueue_scripts', [ $this, 'maybe_enqueue_default_header_menu_script' ], 100 );
	...
	public static function override_d4_fe_scripts(): void {
		...
		// Queue order alone cannot guarantee this listener runs first. An earlier
		// queued scroll or sticky handle can depend on the combined runtime and force
		// WordPress to resolve it before a later Menu handle.
		if (
			wp_script_is( 'divi-script-library-menu', 'enqueued' )
			&& isset( $wp_scripts->registered[ $combined_handle ] )
			&& ! in_array( 'divi-script-library-menu', $wp_scripts->registered[ $combined_handle ]->deps, true )
		) {
			$wp_scripts->registered[ $combined_handle ]->deps[] = 'divi-script-library-menu';
		}
		...
		$pb_custom_data = [
			...
			'slider_swipe'           => __( 'SWIPE →', 'et_builder_5' ),
	...
	/**
	 * Enqueue the D5 Menu runtime when the default PHP header is rendered.
	 *
	 * The default theme header outputs `#top-menu.nav` with `wp_nav_menu()`, not a
	 * `divi/menu` block. Dynamic Assets loads the D5 Menu runtime for Menu modules,
	 * so it never loads that runtime for this header. The runtime adds
	 * `et-reverse-direction-nav`. Without it, a nested header submenu near the
	 * right edge of the viewport can open off screen. A Menu Module on the same
	 * page can load the same runtime and hide the problem.
	 *
	 * This uses the existing `divi-script-library-menu` handle. WordPress will not
	 * enqueue the script twice if a Menu Module also requests it. Menu and Fullwidth
	 * Menu modules still load the script through Dynamic Assets.
	 *
	 * @since ??
	 *
	 * @return void
	 */
	public static function maybe_enqueue_default_header_menu_script(): void {
		// `initialize()` already skips this when D5 is off. Check again so a direct
		// call does not enqueue the D5 runtime on top of D4's `et_fix_nav_direction()`.
		if ( ! Conditions::is_d5_enabled() ) {
			return;
		}

		// The Visual Builder top window does not render the frontend header. Skip the
		// script there. The app window still renders the header and needs this runtime.
		if ( Conditions::is_vb_top_window() ) {
			return;
		}

		// The Divi theme enqueues this handle only when it uses its default-header
		// menu script. Third-party themes do not ship this handle, so they skip too.
		if ( ! wp_script_is( 'divi-theme-scripts-library-menu', 'enqueued' ) ) {
			return;
		}

		// The Divi Builder plugin can run without the Divi theme. In that case there
		// is no default PHP header.
		if ( ! function_exists( 'et_is_theme_builder_header_used' ) ) {
			return;
		}

		// A Theme Builder header replaces the PHP header, so `#top-menu.nav` is not
		// on the page. Checking the theme's default-header script is not enough:
		// two of its three cases still enqueue that script when a Theme Builder
		// header is used in preview and in the Customizer.
		if ( et_is_theme_builder_header_used() ) {
			return;
		}

		// Match `header.php`: it prints `#top-menu-nav` when the header style is not
		// `slide` or `fullscreen`, or when the Customizer preview is active.
		// `header.php` sets `$et_slide_header` after `wp_head()` runs. Read the
		// `header_style` option when that global is not set yet.
		global $et_slide_header;

		if ( null === $et_slide_header ) {
			$header_style    = et_get_option( 'header_style', 'left' );
			$is_slide_header = 'slide' === $header_style || 'fullscreen' === $header_style;
		} else {
			$is_slide_header = (bool) $et_slide_header;
		}

		if ( $is_slide_header && ! is_customize_preview() ) {
			return;
		}
