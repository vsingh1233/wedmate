...
class Page {
	...
	public static function custom_css( $post_id = 0 ) {
		...
		if ( isset( $page_settings['et_pb_content_area_background_color'] ) ) {
			...
			if ( in_array( $post_type, [ ET_THEME_BUILDER_HEADER_LAYOUT_POST_TYPE, ET_THEME_BUILDER_FOOTER_LAYOUT_POST_TYPE ], true ) ) {
				$output .= sprintf(
					'%2$s .et_builder_inner_content { background-color: %1$s; }',
					esc_html( $content_area_bg_color ),
					esc_html( $selector_prefix )
				);
			} else {
				// For Divi Builder layouts (pages, posts, and other singulars with et_pb_pagebuilder_layout).
				$output .= sprintf(
					' .et_pb_pagebuilder_layout #main-content { background-color: %1$s; }',
					esc_html( $content_area_bg_color )
				);
