...
class ScriptData {
	...
	public static function add_data_item( array $args ): bool {
		// Conditional Display always-renders for styles, then discards HTML. Skip ScriptData
		// registration so hidden modules do not leave dangling footer script payloads.
		if ( ModuleRegistration::should_suppress_runtime_assets() ) {
			return false;
		}
	...
	/**
	 * Capture the current script-data store for later restoration.
	 *
	 * Used to isolate nested `do_blocks()` calls (e.g. preset selector registration)
	 * so leftover animation, sticky, multi-view, link, or interaction items from a
	 * fake module cannot attach to reused order-class selectors.
	 *
	 * @since ??
	 *
	 * @return array Snapshot of the script-data store.
	 */
	...
	/**
	 * Restore the script-data store from a previously captured snapshot.
	 *
	 * @since ??
	 *
	...
	public static function restore_state( array $state ): void {
		self::$_script_data = $state['script_data'] ?? self::DEFAULT_SCRIPT_DATA;
	}
}
