...
class Style {
	...
	/**
	 * Capture the current preset-selector processed cache for later restoration.
	 *
	 * Used to isolate side effects of nested `do_blocks()` calls (e.g. preset
	 * selector registration) so that the fake render pass cannot suppress real
	 * preset CSS generation by prematurely marking selectors as processed.
	 *
	 * @since ??
	 *
	 * @return array Snapshot of the preset-selector processed cache.
	 */
	public static function capture_preset_selector_processed_state(): array {
		return self::$_preset_selector_processed;
	}

	/**
	 * Restore the preset-selector processed cache from a previously captured snapshot.
	 *
	 * Reverses premature processed-state mutations introduced by a nested
	 * `do_blocks()` call so the active frontend render pipeline can generate
	 * preset CSS normally.
	 *
	 * @since ??
	 *
	 * @param array $state Snapshot previously returned by `capture_preset_selector_processed_state()`.
	 *
	 * @return void
	 */
	public static function restore_preset_selector_processed_state( array $state ): void {
		self::$_preset_selector_processed = $state;
	}

	/**
	 * Capture style-store state that nested `do_blocks()` calls can mutate.
	 *
	 * Used to isolate fake preset-registration renders from the active frontend
	 * pipeline. Instance CSS and ancestor lookups must not leak after order
	 * indexes and store-instance IDs are restored; preset-group CSS generated
	 * during the isolated pass is re-merged by `restore_state()`.
	 *
	 * @since ??
	 *
	 * @return array Snapshot of style-store state.
	 */
	public static function capture_state(): array {
		return [
			'styles'                    => self::$_styles,
			'ancestor_ids_cache'        => self::$_ancestor_ids_cache,
			'preset_selector_processed' => self::capture_preset_selector_processed_state(),
			'group_style'               => self::$_group_style,
			'style_key_cache'           => self::$_style_key_cache,
			'style_key_cache_context'   => self::$_style_key_cache_context,
		];
	}

	/**
	 * Restore style-store state from a previously captured snapshot.
	 *
	 * Drops leftover instance CSS and ancestor-cache entries from the isolated
	 * pass, then re-applies presetNested / preset / presetGroup CSS collected
	 * during that pass so interaction presets still emit selectors.
	 *
	 * @since ??
	 *
	 * @param array $state Snapshot previously returned by `capture_state()`.
	 *
	 * @return void
	 */
	public static function restore_state( array $state ): void {
		$isolated_preset_styles = self::_extract_preset_style_groups( self::$_styles );

		self::$_styles                  = $state['styles'] ?? [];
		self::$_ancestor_ids_cache      = $state['ancestor_ids_cache'] ?? [];
		self::$_group_style             = $state['group_style'] ?? 'module';
		self::$_style_key_cache         = $state['style_key_cache'] ?? null;
		self::$_style_key_cache_context = $state['style_key_cache_context'] ?? null;

		self::restore_preset_selector_processed_state( $state['preset_selector_processed'] ?? [] );
		self::_merge_isolated_preset_style_groups( $isolated_preset_styles );
	}

	/**
	 * Extract preset-group style data from a `$_styles` snapshot.
	 *
	 * @since ??
	 *
	 * @param array $styles Style store keyed by style key then group.
	 *
	 * @return array presetNested, preset, and presetGroup entries only.
	 */
	private static function _extract_preset_style_groups( array $styles ): array {
		$preset_groups = [];

		foreach ( $styles as $style_key => $groups ) {
			if ( ! is_array( $groups ) ) {
				continue;
			}

			foreach ( [ 'presetNested', 'preset', 'presetGroup' ] as $group ) {
				if ( empty( $groups[ $group ] ) || ! is_array( $groups[ $group ] ) ) {
					continue;
				}

				$preset_groups[ $style_key ][ $group ] = $groups[ $group ];
			}
		}
	...
	/**
	 * Merge preset-group CSS collected during an isolated render back into `$_styles`.
	 *
	 * @since ??
	 *
	 * @param array $isolated_preset_styles Preset groups extracted from the isolated pass.
	 *
	 * @return void
	 */
	private static function _merge_isolated_preset_style_groups( array $isolated_preset_styles ): void {
		foreach ( $isolated_preset_styles as $style_key => $groups ) {
			if ( ! is_array( $groups ) ) {
				continue;
			}

			foreach ( $groups as $group => $group_styles ) {
				if ( empty( $group_styles ) || ! is_array( $group_styles ) ) {
					continue;
				}

				if ( empty( self::$_styles[ $style_key ][ $group ] ) ) {
					self::$_styles[ $style_key ][ $group ] = $group_styles;
					continue;
				}

				self::$_styles[ $style_key ][ $group ] = self::merge_module_styles_data(
					self::$_styles[ $style_key ][ $group ],
					$group_styles
				);
			}
		}
