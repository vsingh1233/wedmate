...
class OrderIndexResetManager {
	...
	/**
	 * Capture the current reset-manager state for later restoration.
	 *
	 * Used to isolate side effects of nested `do_blocks()` calls (e.g. preset
	 * selector registration) from the active frontend render pipeline. Call
	 * `restore_state()` with the returned snapshot to undo any state mutations
	 * introduced by the nested call.
	 *
	 * @since ??
	 *
	 * @return array Snapshot of the current reset-manager state.
	 */
	public static function capture_state(): array {
		return [
			'reset_contexts'                    => self::$_reset_contexts,
			'appended_canvas_processed_layout'  => self::$_appended_canvas_processed_layout,
			'last_reset_layout_type'            => self::$_last_reset_layout_type,
			'previous_layout_type_before_reset' => self::$_previous_layout_type_before_reset,
			'blocks_check_cache'                => self::$_blocks_check_cache,
		];
	}

	/**
	 * Restore the reset-manager state from a previously captured snapshot.
	 *
	 * Reverses any state mutations introduced by a nested `do_blocks()` call so
	 * the active frontend render pipeline continues from its pre-call position.
	 *
	 * @since ??
	 *
	...
	public static function restore_state( array $state ): void {
		self::$_reset_contexts                    = $state['reset_contexts'];
		self::$_appended_canvas_processed_layout  = $state['appended_canvas_processed_layout'];
		self::$_last_reset_layout_type            = $state['last_reset_layout_type'];
		self::$_previous_layout_type_before_reset = $state['previous_layout_type_before_reset'];
		self::$_blocks_check_cache                = $state['blocks_check_cache'];
	}
