...
class Layout {
	...
	public static function render( $layout_type, $layout_id ) {
		...
		// Handle style output.
		// Use the filtered page-resource singular check so integrations (e.g., WooCommerce
		// password-protected category plugins) can prevent a product taxonomy archive from being
		// treated as singular, which would otherwise cause layout rendering to use the main
		// query post ID instead of the layout ID for the static CSS manager.
		if ( et_core_page_resource_is_singular() && ! et_core_is_fb_enabled() ) {
