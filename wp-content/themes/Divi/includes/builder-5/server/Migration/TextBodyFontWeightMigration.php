...
class TextBodyFontWeightMigration extends MigrationContentBase {
	...
	public static function migrate_fe_content(): void {
		...
		if (
			! empty( $tb_template_ids )
			&& false === has_filter( 'et_builder_render_layout', [ __CLASS__, 'migrate_et_builder_render_layout_content' ] )
		) {
			// Gate on the current request hook registry (not a process static) so PHP-FPM
			// workers re-register after `$wp_filter` resets, while WPUnit `wp`/`go_to()`
			// calls still cannot stack duplicate named callbacks.
			add_filter( 'et_builder_render_layout', [ __CLASS__, 'migrate_et_builder_render_layout_content' ], 8 );
		}
	}

	/**
	 * Migrate Theme Builder layout content before `do_blocks()`.
	 *
	 * @since ??
	 *
	 ...
	 * @return string
	 */
	public static function migrate_et_builder_render_layout_content( $content ): string {
		return self::migrate_content_block( $content );
	}
