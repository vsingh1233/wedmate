...
class ModuleUtils {
	...
	public static function remove_matching_values( array $target_attrs, array $reference_attrs ): array {
		...
		foreach ( $target_attrs as $key => $value ) {
			if ( array_key_exists( $key, $reference_attrs ) ) {
				...
				if ( is_array( $value ) && is_array( $reference_value ) ) {
					// Custom attribute rows share name/targetElement across preset and module
					// entries. Only remove a baked duplicate when the entire row matches.
					if ( self::_is_custom_attribute_row( $value ) && self::_is_custom_attribute_row( $reference_value ) ) {
	...
	/**
	 * Whether the array is a Custom Attributes list row.
	 *
	 * @since ??
	 *
	 ...
	 * @return bool
	 */
	private static function _is_custom_attribute_row( $value ): bool {
	 ...
	 *
	 * Group Carousel children are an exception: the carousel has no layout attr (so the default would be
	 * flex), but each child Group is wrapped in a block-level `.et_pb_group_carousel_slide`. The Group is
	 * not a flex item of the track, so parent layout is treated as non-flex and Sizing uses margin alignment.
	 *
	 * This function is equivalent of JS function:
	...
	public static function is_parent_flex_layout( string $module_id, ?int $instance = null ): bool {
		...
		// Carousel children sit in a block-level slide wrapper, not as flex items of the track.
		if ( 'divi/group-carousel' === ( $parent->blockName ?? null ) ) {
			return false;
		}
	...
	public static function add_category_query_args( array $query_args, $categories, string $post_type, int $current_post_id = 0 ): array {
		...
		if ( ! $is_all_category_selected ) {
			// For 'post' type, use 'cat' for categories; tax_query for other post taxonomies (e.g. post_tag).
			if ( 'post' === $post_type ) {
				// Filter out special values and convert 'current' to actual term ID(s).
				$filtered_categories          = [];
				$queried_object               = get_queried_object();
				$archive_taxonomy_for_current = '';
				...
				foreach ( $categories_normalized as $category ) {
					if ( 'current' === $category ) {
						...
						// Tag (and other post-applicable) archives: Current Category must use that term
						// via tax_query — parity with Portfolio project_tag (#29574 / EB-BQ-15).
						if (
							$queried_object instanceof \WP_Term
							&& 'category' !== $queried_object->taxonomy
							&& self::_taxonomy_applies_to_post_type( $queried_object->taxonomy, $post_type )
						) {
							$filtered_categories[]        = (int) $queried_object->term_id;
							$archive_taxonomy_for_current = $queried_object->taxonomy;
							continue;
						}
				...
				if ( '' !== $archive_taxonomy_for_current && ! empty( $filtered_categories ) ) {
					$valid_terms = [];
					foreach ( $filtered_categories as $term_id ) {
						if ( term_exists( (int) $term_id, $archive_taxonomy_for_current ) ) {
							$valid_terms[] = (int) $term_id;
						}
					}
					...
					if ( ! empty( $valid_terms ) ) {
						$query_args['tax_query'] = [
							[
								'taxonomy' => $archive_taxonomy_for_current,
								'field'    => 'id',
								'terms'    => $valid_terms,
								'operator' => 'IN',
							],
				...
				} elseif ( ! empty( $filtered_categories ) ) {
				...
				foreach ( $categories_normalized as $category ) {
					if ( 'current' === $category ) {
						// Prefer the queried archive term when its taxonomy applies to this post type.
						// Covers project_category (#51023) and project_tag (#29574 / EB-BQ-06) without remapping
						// get_taxonomy_for_post_type() globally.
						if (
							$queried_object instanceof \WP_Term
							&& self::_taxonomy_applies_to_post_type( $queried_object->taxonomy, $post_type )
						) {
				...
				// When Current Category resolved from an archive term, use that term's taxonomy (#29574).
				if ( '' !== $archive_taxonomy_for_current ) {
					$taxonomy = $archive_taxonomy_for_current;
				}
	...
	/**
	 * Whether a taxonomy is registered for the given post type.
	 *
	 * Used by Current Category archive resolution so project_tag (and similar) archives
	 * can scope Portfolio queries without remapping get_taxonomy_for_post_type() (#29574).
	 *
	 * @since ??
	 *
	 ...
	 * @return bool
	 */
	private static function _taxonomy_applies_to_post_type( string $taxonomy_name, string $post_type ): bool {
		if ( '' === $taxonomy_name || '' === $post_type || ! taxonomy_exists( $taxonomy_name ) ) {
			return false;
		}
