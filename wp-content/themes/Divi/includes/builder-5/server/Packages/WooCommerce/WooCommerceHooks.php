...
class WooCommerceHooks implements DependencyInterface {
	...
	public function load(): void {
		...
		// Password-protection plugins (e.g., WooCommerce Protected Categories) may force the
		// main query to report as singular on product taxonomy archives. That causes Theme
		// Builder layout rendering to use the wrong post ID for the static CSS manager, which
		// results in missing Theme Builder header/footer styles on protected product category pages.
		add_filter( 'et_core_page_resource_is_singular', [ self::class, 'maybe_prevent_singular_page_resource_for_product_taxonomies' ] );
	...
	/**
	 * Prevent product taxonomy archives from being treated as singular for page resource purposes.
	 *
	 * Password-protection plugins (e.g., WooCommerce Protected Categories) can force the main
	 * query to report as singular on product category/tag pages. When that happens, Theme Builder
	 * layout rendering uses the main query post ID for the static CSS manager instead of the
	 * layout ID, causing the header/footer styles to be missing on the protected page.
	 *
	 * By returning `false` here for product taxonomies, layout rendering falls back to the
	 * layout ID and generates the correct CSS.
	 *
	 * @since ??
	 *
	 * @param bool $is_singular Whether the page is considered singular for page resources.
	 *
	 * @return bool
	 */
	public static function maybe_prevent_singular_page_resource_for_product_taxonomies( bool $is_singular ): bool {
		if ( ! $is_singular ) {
			return false;
		}
	...
	public static function relocate_woocommerce_single_product_summary(): void {
		...
		// Empty layouts must short-circuit (matches et_theme_builder_overrides_layout()).
		// Header/footer-only TB fixtures may omit the body layout key.
		$body_layout               = ! empty( $tb_layouts ) ? ( $tb_layouts[ $tb_body_layout ] ?? null ) : null;
		$tb_body_override          = null !== $body_layout && et_theme_builder_overrides_layout( $tb_body_layout );
		$tb_body_layout_id         = $tb_body_override ? ( $body_layout['id'] ?? false ) : false;
		$tb_body_content           = $tb_body_layout_id ? get_post_field( 'post_content', $tb_body_layout_id ) : '';
