...
class WooCommerceUtils {
	...
	public static function validate_woocommerce_request_params( WP_REST_Request $request ): array {
		...
		$raw_current_page = $request->get_param( 'currentPage' ) ?? [];
		$conditional_tags = $request->get_param( 'conditionalTags' ) ?? [];
		$request_type     = $request->get_param( 'requestType' ) ?? '';
		$main_loop_type   = '';

		if ( $request->has_param( 'mainLoopType' ) ) {
			$main_loop_type = sanitize_key( (string) $request->get_param( 'mainLoopType' ) );
		} elseif ( isset( $raw_current_page['mainLoopType'] ) ) {
			$main_loop_type = sanitize_key( (string) $raw_current_page['mainLoopType'] );
		}

		$raw_main_loop_settings = is_array( $raw_current_page['mainLoopSettingsData'] ?? null )
			? $raw_current_page['mainLoopSettingsData']
			: [];
		...
		$current_page     = array_intersect_key( $raw_current_page, SettingsDataCallbacks::current_page() );

		// Sanitize values.
		$conditional_tags = $utils->sanitize_text_fields( $conditional_tags );
		$current_page     = $utils->sanitize_text_fields( $current_page );
		$request_type     = self::_resolve_woocommerce_rest_request_type(
			sanitize_text_field( (string) $request_type ),
			$main_loop_type
		);

		if ( '' !== $main_loop_type ) {
			$current_page['mainLoopType'] = $main_loop_type;
		}

		if ( ! empty( $raw_main_loop_settings ) ) {
			$current_page['mainLoopSettingsData'] = self::_sanitize_main_loop_settings_data( $raw_main_loop_settings );
		}

		$page_id_raw = $raw_current_page['id'] ?? ( $current_page['id'] ?? null );
		$has_page_id = self::_has_valid_woocommerce_current_page_id( $page_id_raw );
		$product_id  = $has_page_id ? absint( $page_id_raw ) : DynamicAssetsUtils::get_current_post_id();

		$is_non_singular_request = self::_is_non_singular_woocommerce_rest_request( $request_type, $main_loop_type );

		if ( $is_non_singular_request ) {
			// Non-singular archive contexts always require Theme Builder access, even when a numeric page ID is present.
	...
	/**
	 * Resolve WooCommerce REST `requestType` from explicit param or `mainLoopType`.
	 *
	 * @since ??
	 *
	 * @param string $request_type   Explicit request type from the client.
	 * @param string $main_loop_type Main loop type from archive context.
	 *
	 * @return string Resolved request type.
	 */
	private static function _resolve_woocommerce_rest_request_type( string $request_type, string $main_loop_type ): string {
		if ( '' !== $request_type ) {
			return $request_type;
		}
	...
	/**
	 * Whether the WooCommerce REST request targets a non-singular Theme Builder context.
	 *
	 * @since ??
	 *
	 * @param string $request_type   Resolved request type.
	 * @param string $main_loop_type Main loop type from archive context.
	 *
	 * @return bool
	 */
	private static function _is_non_singular_woocommerce_rest_request( string $request_type, string $main_loop_type ): bool {
		if ( in_array( $request_type, [ '404', 'archive', 'home' ], true ) ) {
			return true;
		}

		return '' !== $main_loop_type && 'singular' !== $main_loop_type;
	}

	/**
	 * Whether `currentPage.id` is a valid numeric post ID for singular permission checks.
	 *
	 * @since ??
	 *
	 * @param mixed $page_id_raw Raw page ID from the client payload.
	 *
	 * @return bool
	 */
	private static function _has_valid_woocommerce_current_page_id( $page_id_raw ): bool {
		if ( false === $page_id_raw || null === $page_id_raw || '' === $page_id_raw || 'false' === $page_id_raw ) {
			return false;
		}

		return is_numeric( $page_id_raw ) && 0 < absint( $page_id_raw );
	}

	/**
	 * Sanitize nested `mainLoopSettingsData` while preserving structure for archive hydration.
	 *
	 * @since ??
	 *
	 * @param array $settings Raw main loop settings from the client payload.
	 *
	 * @return array Sanitized settings array.
	 */
	private static function _sanitize_main_loop_settings_data( array $settings ): array {
		$allowed_keys = [
			'termId',
			'taxonomy',
			'termName',
		...
		foreach ( $settings as $key => $value ) {
			...
			if ( ! in_array( $key_string, $allowed_keys, true ) ) {
				continue;
			}

			if ( is_array( $value ) ) {
				$sanitized[ $key_string ] = self::_sanitize_main_loop_settings_data( $value );
				continue;
			}

			if ( is_bool( $value ) ) {
				$sanitized[ $key_string ] = $value;
				continue;
			}

			if ( in_array( $key_string, $numeric_id_keys, true )
				&& ( is_int( $value ) || ( is_string( $value ) && is_numeric( $value ) ) ) ) {
				$sanitized[ $key_string ] = (string) absint( $value );
				continue;
			}

			$sanitized[ $key_string ] = sanitize_text_field( (string) $value );
		}
