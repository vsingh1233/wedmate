...
class Module {
	...
	public static function render( array $args ): string {
			...
			if ( str_contains( $group_id, 'htmlAttributes' )
				...
				foreach ( $html_attrs as $device => $device_data ) {
					...
					if ( isset( $value_data['class'] ) ) {
						$existing_class = $attrs_with_presets['module']['advanced']['htmlAttributes'][ $device ]['value']['class'] ?? '';
						$attrs_with_presets['module']['advanced']['htmlAttributes'][ $device ]['value']['class'] = AttributeUtils::merge_attribute_values(
							'class',
							$existing_class,
							$value_data['class']
						);
		...
		// Wrapper ownership reads resolve through the full preset stack so preset-sourced
		// interactions and custom attributes reach the front-end wrapper. This merge also
		// carries preset style attrs, so it is resolved after CSS ID & Classes above and
		// must not feed that path.
		$ownership_attrs = GlobalPreset::get_merged_attrs(
			[
				'moduleName'  => $name,
				'moduleAttrs' => $attrs,
			]
		...
		$interaction_trigger = $ownership_attrs['module']['decoration']['interactionTrigger'] ?? '';
		$interaction_target  = $ownership_attrs['module']['decoration']['interactionTarget'] ?? '';
		...
		if ( empty( $interaction_trigger ) ) {
			$interactions_data = $ownership_attrs['module']['decoration']['interactions'] ?? null;
		...
		// Add custom attributes if present. Preset-sourced custom attributes are included
		// through `$ownership_attrs` from GlobalPreset::get_merged_attrs().
		$custom_attributes_data = $ownership_attrs['module']['decoration']['attributes'] ?? [];
		...
		if ( is_callable( $script_data_component ) && ! ModuleRegistration::should_suppress_runtime_assets() ) {
			call_user_func(
		...
		if ( ! empty( $attrs['content']['decoration']['inlineFont'] ) && ! ModuleRegistration::should_suppress_runtime_assets() ) {
