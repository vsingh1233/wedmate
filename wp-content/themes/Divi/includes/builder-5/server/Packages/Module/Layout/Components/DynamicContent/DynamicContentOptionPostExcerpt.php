...
class DynamicContentOptionPostExcerpt extends DynamicContentOptionBase implements DynamicContentOptionInterface {
	...
	public function render_callback( $value, array $data_args = [] ): string {
		...
		if ( ! $post ) {
			...
			if ( empty( $value ) ) {
				$strip_shortcode_tokens = static function ( string $content ): string {
					$content = strip_shortcodes( $content );
					// Regex test: https://regex101.com/r/XmrZCo/1
					// Regex unit test: https://regex101.com/r/MDvfNO/1/unit-tests
				...
				$is_generating_excerpt = ! empty( $GLOBALS['divi_generating_excerpt'] );

				if ( et_pb_is_pagebuilder_used( $post->ID ) && ! $is_generating_excerpt ) {
					// For Divi Builder posts, render content before trimming so legacy shortcodes
					// are not treated as excerpt words (#48589). Skip when already inside
					// excerpt generation to prevent the_content → post_excerpt recursion.
					...
					try {
						...
						$value            = $strip_shortcode_tokens( wp_strip_all_tags( $rendered_content ) );
					} finally {
					...
					$value = $strip_shortcode_tokens( wp_strip_all_tags( $post->post_content ) );
				}
