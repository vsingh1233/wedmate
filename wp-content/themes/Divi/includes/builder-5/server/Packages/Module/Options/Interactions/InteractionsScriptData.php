...
class InteractionsScriptData {
	 ...
	 *
	 * State isolation: parser (including layout area), reset-manager, module-order, Style,
	 * and ScriptData store state are captured before the fake `do_blocks()` pass and restored
	 * afterwards. Style restore drops leftover instance CSS / ancestor-cache entries keyed by
	 * reused order classes, then re-merges preset-group CSS so interaction presets still emit
	 * selectors. ScriptData restore drops leftover animation, sticky, multi-view, link, and
	 * interaction items from the fake module so they cannot attach to reused selectors.
	 *
	 * Module-order isolation uses companion APIs `ET_Builder_Module_Order::capture_state()` /
	 * `restore_state()` from submodule-builder (elegantthemes/submodule-builder#10724). Those
	 * APIs must be present: checkout the companion branch locally and merge it before or with
	 * this change. Parser, reset-manager, Style, and ScriptData layers are restored in the
	 * same finally block.
	 *
	 * @since ??
	 *
	...
	private static function _register_preset_selectors_as_used( array $interactions ): void {
		...
		if ( ! empty( $fake_blocks ) ) {
			...
			// Capture parser, reset-manager, module-order, Style, and ScriptData store state
			// before the fake preset rendering pass so that it cannot alter the active page
			// render state. This prevents order-index desynchronization and leftover fake-module
			// script data in Theme Builder off-canvas + preset interaction scenarios.
			$block_parser_state  = BlockParserStore::capture_state();
			$reset_manager_state = OrderIndexResetManager::capture_state();
			$style_state         = Style::capture_state();
			$script_data_state   = ScriptData::capture_state();
			$module_order_state  = \ET_Builder_Module_Order::capture_state();
			...
			try {
				...
				// Always reset flag and restore captured state, even if an exception occurs.
				self::$_registering_preset_selectors = false;

				BlockParserStore::restore_state( $block_parser_state );
				OrderIndexResetManager::restore_state( $reset_manager_state );
				Style::restore_state( $style_state );
				ScriptData::restore_state( $script_data_state );
				\ET_Builder_Module_Order::restore_state( $module_order_state );
			}
		}
	}
}
