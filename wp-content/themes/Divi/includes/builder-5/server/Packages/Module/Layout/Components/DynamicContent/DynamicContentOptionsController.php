...
class DynamicContentOptionsController extends RESTController {
	...
	public static function index_permission( \WP_REST_Request $request ): bool {
		...
		if ( ! UserRole::can_current_user_use_visual_builder() || 0 >= $post_id ) {
			return false;
		}
		...
		// In TBVB / Full Site Editing a module inside a Theme Builder layout area sends its
		// layout post ID (`et_header_layout` / `et_body_layout` / `et_footer_layout`) so the
		// options match the Theme Builder layout editor. Requesting options for a layout post
		// additionally requires the Theme Builder capability.
		if ( et_theme_builder_is_layout_post_type( get_post_type( $post_id ) ) && ! et_pb_is_allowed( 'theme_builder' ) ) {
			return false;
		}
