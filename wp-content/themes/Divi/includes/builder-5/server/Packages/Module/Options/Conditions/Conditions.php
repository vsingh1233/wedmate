...
class Conditions {
	...
	protected function _process_date_time_condition( array $condition_settings ): bool {
		...
		$time       = $this->_sanitize_time_string( $time );
		$from_time  = $this->_sanitize_time_string( $from_time );
		$until_time = $this->_sanitize_time_string( $until_time );
	...
	/**
	 * Sanitize a time string to a valid HH:MM:SS format.
	 *
	 * Repairs incomplete but numeric time strings like '11:', '11', and '11:30'
	 * to full 'HH:MM:SS' format. Empty segments are treated as 0 (e.g. '11:' → '11:00:00').
	 * Preserves empty strings so existing callers can still treat "no time" as a
	 * special case. Returns the default for strings that contain non-numeric or
	 * out-of-range parts.
	 *
	 * @since ??
	 *
	 * @param string $time    The raw time string (e.g. '11:', '11:30', '').
	 * @param string $default The fallback time string. Default '00:00:00'.
	 *
	 * @return string Sanitized time string in HH:MM:SS format, or '' if input was empty.
	 */
	protected function _sanitize_time_string( string $time, string $default = '00:00:00' ): string {
		$time = trim( $time );
		...
		// Non-numeric segments (e.g. 'abc') make the whole string invalid.
		if ( ( '' !== $hour_str && ! ctype_digit( $hour_str ) )
			|| ( '' !== $minute_str && ! ctype_digit( $minute_str ) )
			|| ( '' !== $second_str && ! ctype_digit( $second_str ) )
		) {
			return $default;
		}

		// Empty segments are treated as 0, not as invalid.
		$hour   = '' !== $hour_str ? (int) $hour_str : 0;
		$minute = '' !== $minute_str ? (int) $minute_str : 0;
		$second = '' !== $second_str ? (int) $second_str : 0;
