...
class LoopUtils {
	...
	/**
	 * Internal {@see \WP_Query} argument: module `postOffset` (base only) used to correct
	 * `found_posts` / `max_num_pages` when offset and pagination are combined.
	 *
	 * Distinct from SQL `offset`, which is `postOffset + (page - 1) * postPerPage`.
	 *
	 * @since ??
	 */
	public const LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR = 'et_builder_loop_pagination_base_offset';
	...
	public static function get_query_args_from_attrs( $attrs ) {
		...
		// Preserve module base offset separately from the compound SQL offset used below.
		$pagination_base_offset = $post_offset;
		...
		$params = [
			...
			'pagination_base_offset'           => $pagination_base_offset,
	...
	private static function _build_post_query_args( $params ) {
		...
		// Carry base `postOffset` separately from compound SQL offset so found_posts
		// correction subtracts the module offset only (Codex offset + pagination).
		$pagination_base_offset = isset( $params['pagination_base_offset'] )
			? absint( $params['pagination_base_offset'] )
			: 0;

		if ( 0 < $pagination_base_offset ) {
			$query_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ] = $pagination_base_offset;
		}
	...
	private static function _execute_post_query( $query_args ) {
		$pagination_base_offset = isset( $query_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ] )
			? (int) $query_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ]
			: 0;

		$should_apply_loop_offset_found_posts = 0 < $pagination_base_offset;

		if ( $should_apply_loop_offset_found_posts ) {
			add_filter( 'found_posts', [ self::class, 'filter_found_posts_for_loop_offset' ], 10, 2 );
		}
		...
		try {
			...
			if ( $should_apply_loop_offset_found_posts ) {
				remove_filter( 'found_posts', [ self::class, 'filter_found_posts_for_loop_offset' ], 10 );
			}
		}
	...
	/**
	 * Reduce {@see \WP_Query::found_posts} for Loop post queries that use a base post offset,
	 * so `max_num_pages` matches Codex guidance for offset + pagination.
	 *
	 * Subtracts the module `postOffset` only. The SQL `offset` may already include the
	 * pagination shift `(page - 1) * postPerPage` and must not be subtracted here.
	 *
	 * @since ??
	 *
	 ...
	 * @return int
	 */
	public static function filter_found_posts_for_loop_offset( $found_posts, $query ) {
		$base_offset = (int) $query->get( self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR );
	 ...
	 * @since ??
	 *
	 * @param int $post_id Optional post ID whose content should be inspected.
	 *
	 * @return bool True if paginated loops detected, false otherwise.
	 */
	public static function has_paginated_loops( int $post_id = 0 ): bool {
		if ( 0 === $post_id ) {
	 ...
	 */
	public static function current_page_has_random_order_loops( int $post_id = 0 ): bool {
		if ( 0 < $post_id ) {
			$post = get_post( $post_id );
			...
			return self::_content_has_random_order_loops( $post->post_content );
		}
	 ...
	 * `orderby`, `order`, non-empty `meta_query`, and the Loop pagination base-offset marker
	 * from `_build_post_query_args()` so archive context stays intact while user-selected
	 * ordering and Codex part-2 page counts are preserved.
	 *
	 * @since ??
	 *
	...
	private static function _merge_current_page_loop_sort_args( array $built_args, array $prior_args ): array {
		...
		if ( isset( $prior_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ] ) ) {
			$built_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ] = (int) $prior_args[ self::LOOP_PAGINATION_BASE_OFFSET_QUERY_VAR ];
		}
