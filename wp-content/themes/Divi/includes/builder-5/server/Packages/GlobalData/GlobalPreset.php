...
class GlobalPreset {
	...
	/**
	 * When true, nested `save_data()` calls also skip third-party page-cache purge.
	 *
	 * Preset migrations call `save_data()` with the default false. Conversion import
	 * sets this for the duration of `execute_preset_migrations()` so those nested
	 * saves cannot undo a conversion-scoped skip (see #49467).
	 *
	 * @since ??
	 *
	 * @var bool
	 */
	private static $_skip_external_cache_nested = false;
	 ...
	 * @param bool  $skip_external_cache Whether to skip third-party page-cache purge. Default false.
	 *
	 * @return array The saved data. The array structure is aligns with GlobalData.Presets.Items TS interface.
	 */
	public static function save_data( array $data, bool $skip_external_cache = false ): array {
		// Nested preset migrations inherit the conversion skip (see #49467).
		$skip_external_cache = $skip_external_cache || self::$_skip_external_cache_nested;
		...
		ET_Core_PageResource::remove_static_resources( 'all', 'all', true, 'all', true, false, $skip_external_cache );
	...
	public static function save_is_legacy_presets_imported( bool $data ): void {
		...
		// We need to clear the entire website cache when updating a preset.
		// Preserve VB CSS files to prevent visual builder from losing its styles.
		// Skip the third-party page-cache purge. The D4 conversion reaches this on
		// Visual Builder open, and the Rollback endpoint runs its own unscoped
		// remove_static_resources() right after calling this (see #49467).
		ET_Core_PageResource::remove_static_resources( 'all', 'all', true, 'all', true, false, true );
	}
	...
	public static function get_module_preset_class_names( array $args ): array {
		...
		if ( empty( $preset_ids ) ) {
			$default_preset_item = ! empty( $default_preset_id )
				? ( $all_data['module'][ $module_name_converted ]['items'][ $default_preset_id ] ?? null )
				: null;

			// Only apply default preset class when default preset has attrs or styleAttrs content.
			if ( is_array( $default_preset_item ) && self::_preset_has_content( $default_preset_item ) ) {
	...
	public static function maybe_convert_legacy_presets(): bool {
		...
		$import_result                           = self::process_presets_for_import( $d4_presets, true, true );
	 ...
	 */
	public static function process_presets_for_import( array $presets, bool $auto_save = true, bool $skip_external_cache = false ): array {
		if ( empty( $presets ) ) {
			return [
		...
		if ( $auto_save ) {
			// Conversion paths set $skip_external_cache true. Preset migrations call
			// save_data() with the default false, so hold the nested skip for that
			// window so they cannot undo the conversion skip (see #49467).
			if ( $skip_external_cache ) {
				self::$_skip_external_cache_nested = true;
			}

			try {
				self::save_data( $final_presets, $skip_external_cache );
				Migration::get_instance()->execute_preset_migrations();
				$final_presets = self::get_data();
			} finally {
				self::$_skip_external_cache_nested = false;
			}
		}
