...
class GradientUtils {
	...
	public static function get_effective_gradient_for_breakpoint( array $args ): array {
		...
		$effective_gradient = array_merge(
	...
	private static function _is_safe_relative_hsl_function( string $color, int $depth = 0 ): bool {
		...
		// Hue/lightness: plain calc(h|l ± N). Saturation also allows Divi's Color Filter clamp max(0, calc(s ± N)).
		// Regex test: https://regex101.com/r/UBIWS7/2/unit-tests.
		if (
			! is_string( $h_component ) || 1 !== preg_match( '/^calc\(\s*h\s*[+\-]\s*-?(?:\d+|\d+\.\d+|\.\d+)(?:deg)?\s*\)$/i', $h_component ) ||
			! is_string( $s_component ) || 1 !== preg_match( '/^(?:calc\(\s*s\s*[+\-]\s*-?(?:\d+|\d+\.\d+|\.\d+)%?\s*\)|max\(\s*0\s*,\s*calc\(\s*s\s*[+\-]\s*-?(?:\d+|\d+\.\d+|\.\d+)%?\s*\)\s*\))$/i', $s_component ) ||
	...
	public static function gradient_style_declaration( array $gradient ): string {
		...
		if ( count( $stops ) >= 2 ) {
			...
			$sanitized = self::_sanitize_css_property_value( 'background-image', $final_css );
		}
