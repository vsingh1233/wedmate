...
class PostFilterCustomFieldValueOptions {
	...
	public static function flush_cache_on_meta_change( $meta_id, int $object_id, string $meta_key ): void {
		...
		if ( ! is_string( $post_type ) || ! self::should_flush_cache_for_post_type( $post_type ) ) {
			return;
		}
	...
	public static function flush_cache_on_post_status_change( string $new_status, string $old_status, $post ): void {
		...
		if ( ! self::should_flush_cache_for_post_type( $post->post_type ) ) {
			return;
		}
	...
	/**
	 * Whether cache invalidation should run for a post type.
	 *
	 * Automatic option discovery only queries public loop sources (and pages).
	 * Non-public types such as WooCommerce shop_order must not scan wp_options.
	 *
	 * @since ??
	 *
	 ...
	 * @return bool
	 */
	private static function should_flush_cache_for_post_type( string $post_type ): bool {
		return '' !== $post_type && et_builder_is_post_type_public( $post_type );
	}
