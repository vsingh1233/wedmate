...
class TestimonialModule implements DependencyInterface {
	...
	/**
	 * Style declaration that stacks Testimonial inner content above background overlays.
	 *
	 * Overlay siblings (mask, pattern, parallax, video) are position:absolute with no z-index,
	 * so they paint on top of in-flow `.et_pb_testimonial_inner`. This matches the parallax
	 * content stacking contract on that existing wrapper without changing overlay markup.
	 *
	 * This function is equivalent of JS function:
	 * {@link /docs/builder-api/js/module-library/testimonial-content-stacking-style-declaration contentStackingStyleDeclaration}
	 * located in `@divi/module-library` package.
	 *
	 * @since ??
	 *
	 ...
	 * }
	 *
	 * @return string The stacking CSS declaration.
	 */
	public static function content_stacking_style_declaration( array $params ): string {
		$style_declarations = new StyleDeclarations( $params );
	...
	/**
	 * Style declaration that keeps the hanging quote icon uncropped when background video is on.
	 *
	 * Shared background-video CSS sets overflow:hidden on the module host, which clips
	 * `{{selector}}:before`. This Testimonial-only override is gated by `.et-pb-has-background-video`.
	 *
	 * This function is equivalent of JS function:
	 * {@link /docs/builder-api/js/module-library/testimonial-video-overflow-style-declaration videoOverflowStyleDeclaration}
	 * located in `@divi/module-library` package.
	 *
	 * @since ??
	 *
	 ...
	 * }
	 *
	 * @return string The overflow CSS declaration.
	 */
	public static function video_overflow_style_declaration( array $params ): string {
		$style_declarations = new StyleDeclarations( $params );

		$style_declarations->add( 'overflow', 'visible' );

		return $style_declarations->value();
	}

	/**
	 * Style declaration that keeps the background video clipped to the module border radius.
	 *
	 * After the host overflow is visible, the video layer must inherit border-radius so
	 * rounded corners still clip the video rather than the hanging quote icon.
	 *
	 * This function is equivalent of JS function:
	 * {@link /docs/builder-api/js/module-library/testimonial-video-border-radius-style-declaration videoBorderRadiusStyleDeclaration}
	 * located in `@divi/module-library` package.
	 *
	 * @since ??
	 *
	 ...
	 * }
	 *
	 * @return string The border-radius CSS declaration.
	 */
	public static function video_border_radius_style_declaration( array $params ): string {
		$style_declarations = new StyleDeclarations( $params );
	...
	public static function module_styles( array $args ): void {
		...
		// Layout is only the print trigger so stacking/video overrides always emit.
		$stacking_print_trigger_attr = $attrs['module']['decoration']['layout'] ?? [];

		if ( empty( $stacking_print_trigger_attr ) ) {
			$stacking_print_trigger_attr = [
				'desktop' => [
					'value' => [],
				],
			];
							...
							'styleProps' => [
								...
								'advancedStyles'           => [
									...
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$args['orderClass']} .et_pb_testimonial_inner",
											'attr'     => $stacking_print_trigger_attr,
											'declarationFunction' => [ self::class, 'content_stacking_style_declaration' ],
										],
									],
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$args['orderClass']}.et-pb-has-background-video",
											'attr'     => $stacking_print_trigger_attr,
											'declarationFunction' => [ self::class, 'video_overflow_style_declaration' ],
										],
									],
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$args['orderClass']} > .et-pb-background-video",
											'attr'     => $stacking_print_trigger_attr,
											'declarationFunction' => [ self::class, 'video_border_radius_style_declaration' ],
										],
									],
								],
							],
