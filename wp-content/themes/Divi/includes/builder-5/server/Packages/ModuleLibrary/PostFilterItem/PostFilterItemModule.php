...
class PostFilterItemModule implements DependencyInterface {
	/**
	 * Get attributes from the nearest Post Filter ancestor.
	 *
	 * @since ??
	 *
	 * @param string   $child_id       Child module id.
	 * @param int|null $store_instance Optional store instance.
	 *
	 * @return array
	 */
	private static function _get_post_filter_ancestor_attrs( string $child_id, $store_instance = null ): array {
		$post_filter = BlockParserStore::find_ancestor(
			$child_id,
			static function ( $block ) {
				return isset( $block->blockName ) && 'divi/post-filter' === $block->blockName;
			},
			$store_instance
		);
		...
		return is_array( $post_filter->attrs ?? null ) ? $post_filter->attrs : [];
	}
	...
	public static function module_script_data( array $args ): void {
		...
		$post_filter_attrs = self::_get_post_filter_ancestor_attrs( $id, $store_instance );
		$target_loop       = $post_filter_attrs['module']['advanced']['targetLoop']['desktop']['value'] ?? '';
			...
			[
				...
				'setContent'    => [
					...
					[
						...
						'valueResolver' => function ( $value ) use ( $post_type, $attrs, $order_by_context, $target_loop, $store_instance, $id ) {
	...
	public static function render_callback( array $attrs, string $child_modules_content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		$post_filter_attrs     = self::_get_post_filter_ancestor_attrs(
