...
class ButtonModule implements DependencyInterface {
	...
	/**
	 * Button alignment for the button element (text-align + margin-left/right).
	 *
	 * Author `text-align` overrides UA `text-align: center` on `<button>` so Module
	 * Alignment still positions the label when the control is full-width.
	 *
	 * @since ??
	 *
	 * @param array $params An array of arguments.
	 *
	 * @return string The CSS for button element alignment.
	 */
	public static function button_alignment_element_declaration( array $params ): string {
		$alignment_attr = $params['attrValue'];
		...
		if ( $alignment_attr ) {
			// Override UA text-align on <button>; also matches inherited wrapper text-align for <a>.
			$style_declarations->add( 'text-align', $alignment_attr );
	...
	/**
	 * Backward-compatible alias for `button_alignment_element_declaration()`.
	 *
	 * @since ??
	 *
	 * @deprecated Use `button_alignment_element_declaration()` instead.
	 *
	 * @param array $params An array of arguments.
	 *
	 * @return string The CSS for button element alignment.
	...
	public static function button_alignment_anchor_declaration( array $params ): string {
		return self::button_alignment_element_declaration( $params );
	}

	/**
	 * Sizing Module Alignment text-align for the button element only.
	 *
	 * Reads `alignment` from the sizing attr value. Kept separate from shared
	 * `ButtonDeclaration::style_declaration` so future shared declaration changes
	 * cannot accidentally land on the element selector.
	 *
	 * @since ??
	 *
	 * @param array $params An array of arguments.
	 *
	 * @return string The generated text-align declaration for the button element.
	 */
	public static function button_sizing_alignment_declaration( array $params ): string {
								...
								'advancedStyles' => [
									...
									[
										...
										'props'         => [
											...
											'attr'     => $attrs['module']['advanced']['alignment'] ?? [],
											'declarationFunction' => [ self::class, 'button_alignment_element_declaration' ],
										],
									],
									[
										'componentName' => 'divi/common',
						...
						[
							...
							'styleProps' => [
								...
								'advancedStyles' => [
									...
									[
										// Element text-align for sizing Module Alignment (overrides UA center on <button>).
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$wrapper_order_class} {$base_order_class}",
											'attr'     => $attrs['button']['decoration']['sizing'] ?? [],
											'declarationFunction' => [ self::class, 'button_sizing_alignment_declaration' ],
										],
									],
									[
										'componentName' => 'divi/common',
	...
	public static function icon_style_declaration( array $params ): string {
		...
		if ( ! empty( $settings['type'] ) ) {
			$font_family = Utils::get_icon_font_family( $settings['type'] ?? null );
