...
class SliderModule implements DependencyInterface {
	...
	public static function module_classnames( $args ) {
		...
		$control_style           = $attrs['pagination']['advanced']['style']['desktop']['value'] ?? 'dot';
		$show_counter            = $attrs['pagination']['advanced']['showCounter']['desktop']['value'] ?? 'on';
		$show_swipe_label        = $attrs['pagination']['advanced']['showSwipeLabel']['desktop']['value'] ?? 'on';
		...
		$classnames_instance->add( 'et_pb_slider_bottom_controls', 'on' === $show_pagination && 'swipe' === $control_style );
		$classnames_instance->add(
			'et_pb_slider_bottom_controls_no_counter',
			'on' === $show_pagination && 'swipe' === $control_style && 'on' !== $show_counter
		);
		$classnames_instance->add(
			'et_pb_slider_bottom_controls_no_swipe',
			'on' === $show_pagination && 'swipe' === $control_style && 'on' !== $show_swipe_label
		);
	...
	/**
	 * Build HTML attributes for slider bottom-control runtime options.
	 *
	 * @since ??
	 *
	 * @param array $attrs Module attributes.
	 *
	 * @return array HTML attributes for the slider wrapper.
	 */
	public static function module_html_attrs( $attrs ) {
		$show_pagination  = $attrs['pagination']['advanced']['show']['desktop']['value'] ?? 'on';
		$control_style    = $attrs['pagination']['advanced']['style']['desktop']['value'] ?? 'dot';
		$html_attrs       = [];

		if ( 'on' !== $show_pagination || 'swipe' !== $control_style ) {
			return $html_attrs;
		}

		$swipe_text       = $attrs['pagination']['advanced']['swipeText']['desktop']['value'] ?? '';
		$show_counter     = $attrs['pagination']['advanced']['showCounter']['desktop']['value'] ?? 'on';
		$show_swipe_label = $attrs['pagination']['advanced']['showSwipeLabel']['desktop']['value'] ?? 'on';

		if ( ! empty( $swipe_text ) ) {
			// HTMLUtility::render() already escapes attribute values. Do not pre-escape.
			$html_attrs['data-swipe-text'] = $swipe_text;
		}

		if ( 'on' !== $show_counter ) {
			$html_attrs['data-show-counter'] = 'off';
		}

		if ( 'on' !== $show_swipe_label ) {
			$html_attrs['data-show-swipe-label'] = 'off';
		}
		...
		MultiViewScriptData::set(
			[
				...
				'setVisibility' => [
					...
					[
						'selector'      => $selector . ' .et-pb-controllers, ' . $selector . ' .et-pb-slider-bottom-controls',
	...
	public static function render_callback( $attrs, $content, $block, $elements, $default_printed_style_attrs ) {
		...
		return Module::render(
			[
				...
				'htmlAttrs'                => self::module_html_attrs( $attrs ),
									...
									[
										...
										'props'         => [
											'selector' => implode(
												', ',
												[
													"{$args['orderClass']}.et_pb_slider_bottom_controls .et-pb-slider-bottom-controls-counter",
													"{$args['orderClass']}.et_pb_slider_bottom_controls .et-pb-slider-bottom-controls-swipe",
												]
											),
											'attr'     => $attrs['pagination']['advanced']['color'] ?? [],
											'property' => 'color',
										],
									],
									[
	...
	public function load() {
		...
		add_filter( 'divi.conversion.postConvertAttrs', [ self::class, 'add_migration_pagination_style' ], 10, 4 );
	...
	/**
	 * Preserve classic dot navigation for layouts converted from Divi 4.
	 *
	 * Native Divi 5 sliders that omit this attribute also keep dots via the render-time fallback.
	 *
	 * @since ??
	 *
	 * @param array  $converted_attrs      Converted module attributes.
	 * @param string $module_name          Module name.
	 * @param array  $original_attrs       Original shortcode attributes.
	 * @param bool   $is_preset_conversion Whether this is a preset conversion.
	 *
	 * @return array Updated converted attributes.
	 */
	public static function add_migration_pagination_style( array $converted_attrs, string $module_name, array $original_attrs, bool $is_preset_conversion ): array {
		if ( 'divi/slider' !== $module_name || $is_preset_conversion ) {
			return $converted_attrs;
		}

		if ( ! isset( $converted_attrs['pagination']['advanced']['style']['desktop']['value'] ) ) {
			$converted_attrs['pagination']['advanced']['style']['desktop']['value'] = 'dot';
		}
