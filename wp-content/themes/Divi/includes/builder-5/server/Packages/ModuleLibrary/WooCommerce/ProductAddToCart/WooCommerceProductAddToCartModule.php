...
class WooCommerceProductAddToCartModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements ): string {
		...
		$stored_product_value = $block->parsed_block['attrs']['content']['advanced']['product'][ $default_breakpoint ][ $default_state ] ?? null;

		// Use stored block attrs (before default merge) so shop-default `dynamic` → `latest` stays contextual.
		$is_contextual_product = null === $stored_product_value
			|| in_array( $stored_product_value, [ 'current', 'dynamic' ], true );

		// Use the loop item product when a contextual product selection is used inside a loop.
		if ( $loop_product_id > 0 && $is_contextual_product ) {
	...
	public static function button_icon_style_declaration( array $params ): string {
		...
		if ( 'off' !== $enable && ! empty( $icon_settings ) && Utils::process_font_icon( $icon_settings ) ) {
			$font_family    = Utils::get_icon_font_family( $icon_settings['type'] ?? null );
