...
class LinkModule implements DependencyInterface {
	...
	/**
	 * Content stacking style declaration for the Link module.
	 *
	 * Lifts `.et_pb_link_inner` above background overlays (mask, pattern, parallax,
	 * video). Those layers are `position: absolute` siblings rendered before the
	 * inner wrapper; without a stacking context they paint over icon and text.
	 *
	 * Always emit stacking. Do not key off overlay attrs — overlays can appear in
	 * any combination, and the inner wrapper should stay above them whenever they
	 * exist. Matches the `.et-pb-parallax-content` contract so mask/pattern work
	 * as well as parallax/video.
	 *
	 * @since ??
	 *
	 * @param array $params Style declaration params.
	 *
	 * @return string The CSS for inner content stacking.
	 */
