...
class PostNavigationModule implements DependencyInterface {
	 ...
	 * Get current post ID.
	 *
	 * Theme Builder layouts replace the global post with the layout CPT during render.
	 * Prefer the queried singular post (#40490), then the main-query post under TB so
	 * adjacent navigation never anchors on `et_*_layout` (#45665).
	 *
	 * @since ??
	 *
	...
	protected static function _get_current_post_id() {
		...
		// Prefer the viewed singular post ID (#40490). Ignore archive term IDs.
		...
		if ( $is_theme_builder && class_exists( '\ET_Post_Stack' ) ) {
			...
			// No content post under TB — do not fall back to the spoofed layout CPT.
			return 0;
		}
