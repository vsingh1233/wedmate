...
class PortfolioModule implements DependencyInterface {
	...
	public static function overlay_icon_style_declaration( array $params ): string {
		...
		if ( isset( $overlay_icon_attr['type'] ) && ! empty( $overlay_icon_attr['type'] ) ) {
			$font_family = IconFontUtils::get_icon_font_family( $overlay_icon_attr['type'] ?? null );
									...
									'propertySelectors' => [
										'desktop' => [
											'value' => [
												...
												'height'       => "{$order_class} .et_portfolio_image img",
												'min-height'   => "{$order_class} .et_portfolio_image img",
												'max-height'   => "{$order_class} .et_portfolio_image img",
											],
										],
									],
								],
