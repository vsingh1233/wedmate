...
class WooCommerceProductsModule implements DependencyInterface {
	...
	/**
	 * REST Use Current Loop context consumed by shortcode filter callbacks.
	 *
	 * @since ??
	 *
	 * @var array{search_query?: string, taxonomy?: string, term_id?: int}
	 */
	public static array $rest_loop_context = [];
	...
	public static function get_shop( array $args = [], array $conditional_tags = [], array $current_page = [] ): string {
		...
		$product_categories = [];
		$product_tags       = [];
		$main_loop_type     = isset( $current_page['mainLoopType'] ) ? sanitize_key( (string) $current_page['mainLoopType'] ) : '';
		$main_loop_settings = isset( $current_page['mainLoopSettingsData'] ) && is_array( $current_page['mainLoopSettingsData'] )
			? $current_page['mainLoopSettingsData']
			: [];
		$wants_current_loop = 'on' === ArrayUtility::get_value( $args, 'use_current_loop', 'off' );
		$live_loop_context  = is_post_type_archive( 'product' ) || is_search() || et_is_product_taxonomy();
		$rest_loop_context  = $wants_current_loop && ! $live_loop_context
			&& self::_is_rest_product_loop_context( $main_loop_type, $main_loop_settings );
		$use_current_loop   = $wants_current_loop && ( $live_loop_context || $rest_loop_context );
		$product_attribute  = '';
		$product_terms      = [];
		...
		if ( $use_current_loop ) {
			...
			} elseif ( $rest_loop_context ) {
				if ( 'search' === $main_loop_type ) {
					$search_query = self::_resolve_rest_search_query( $current_page, $main_loop_settings );

					if ( '' !== $search_query ) {
						self::$rest_loop_context['search_query'] = $search_query;
					}
				} else {
					self::_apply_rest_product_loop_from_settings(
						$main_loop_type,
						$main_loop_settings,
						$include_categories,
						$product_tags,
	...
	public static function filter_products_query( array $query_args ): array {
		$rest_search_query = sanitize_text_field( (string) ( self::$rest_loop_context['search_query'] ?? '' ) );

		if ( '' !== $rest_search_query ) {
			$query_args['s'] = $rest_search_query;
		} elseif ( is_search() ) {
		...
		$rest_taxonomy = sanitize_key( (string) ( self::$rest_loop_context['taxonomy'] ?? '' ) );
		$rest_term_id  = absint( self::$rest_loop_context['term_id'] ?? 0 );

		if ( '' !== $rest_taxonomy && 0 < $rest_term_id ) {
			$rest_tax_clause = [
				'taxonomy' => $rest_taxonomy,
				'field'    => 'id',
				'terms'    => $rest_term_id,
			];

			if ( is_array( $query_args['tax_query'] ?? null ) ) {
				$query_args['tax_query'][] = $rest_tax_clause;
			} else {
				$query_args['tax_query'] = [ $rest_tax_clause ];
			}
		}
	...
	/**
	 * Resolve the raw search query for REST Use Current Loop contexts.
	 *
	 * @since ??
	 *
	 * @param array $current_page         Current page settings from the client payload.
	 * @param array $main_loop_settings   Main loop settings from the client payload.
	 *
	 * @return string Sanitized search query string.
	 */
	private static function _resolve_rest_search_query( array $current_page, array $main_loop_settings ): string {
		if ( isset( $main_loop_settings['searchQuery'] ) ) {
			return sanitize_text_field(
				wp_specialchars_decode( (string) $main_loop_settings['searchQuery'], ENT_QUOTES )
			);
		}

		if ( ! isset( $current_page['title'] ) ) {
			return '';
		}

		return sanitize_text_field(
			wp_specialchars_decode( (string) $current_page['title'], ENT_QUOTES )
		);
	}

	/**
	 * Whether REST archive context represents a WooCommerce product loop.
	 *
	 * @since ??
	 *
	 * @param string $main_loop_type     Main loop type from current page settings.
	 * @param array  $main_loop_settings Main loop settings from current page settings.
	 *
	 * @return bool
	 */
	private static function _is_rest_product_loop_context( string $main_loop_type, array $main_loop_settings ): bool {
		if ( '' === $main_loop_type || 'singular' === $main_loop_type ) {
			return false;
		}

		if ( 'post_type_archive' === $main_loop_type ) {
			$post_type = isset( $main_loop_settings['postType'] ) ? sanitize_key( (string) $main_loop_settings['postType'] ) : '';

			return '' === $post_type || 'product' === $post_type;
		}

		if ( in_array( $main_loop_type, [ 'category', 'tag', 'taxonomy' ], true ) ) {
			$taxonomy = isset( $main_loop_settings['taxonomy'] ) ? sanitize_key( (string) $main_loop_settings['taxonomy'] ) : '';
			...
			return in_array( $taxonomy, $product_taxonomies, true );
		}
	...
	/**
	 * Apply product archive loop filters from REST `mainLoopSettingsData`.
	 *
	 * @since ??
	 *
	 * @param string $main_loop_type       Main loop type from current page settings.
	 * @param array  $main_loop_settings   Main loop settings from current page settings.
	 * @param array  $include_categories   Category include list (by reference).
	 * @param array  $product_tags         Product tag slugs (by reference).
	 * @param string $product_attribute    Product attribute taxonomy (by reference).
	 * @param array  $product_terms        Product attribute term slugs (by reference).
	 *
	 * @return void
	 */
	private static function _apply_rest_product_loop_from_settings(
		string $main_loop_type,
		array $main_loop_settings,
		...
		$term_id  = absint( $main_loop_settings['termId'] ?? 0 );
		$taxonomy = isset( $main_loop_settings['taxonomy'] ) ? sanitize_key( (string) $main_loop_settings['taxonomy'] ) : '';
		...
		if ( 'product_cat' === $taxonomy ) {
			$include_categories = [ (string) $term_id ];
			return;
		}
		...
		if ( StringUtility::starts_with( $taxonomy, 'pa_' ) ) {
			$product_attribute = $taxonomy;
			$product_terms[]   = $term->slug;
			return;
		}
		...
		if ( in_array( $taxonomy, $product_taxonomies, true ) ) {
			self::$rest_loop_context['taxonomy'] = $taxonomy;
			self::$rest_loop_context['term_id']  = $term_id;
		}
	}
}
