...
use ET\Builder\Packages\ModuleLibrary\LabelTextDecorationFontProperties;
...
class ContactFormModule implements DependencyInterface {
	...
	public static function module_styles( array $args ): void {
		...
		$field_label_targets                       = [
			"{$order_class} .et_pb_contact_field .et_pb_contact_form_label",
		];
		$field_label_targets_hover                 = [
			"{$order_class} .et_pb_contact_field .et_pb_contact_form_label:hover",
		];
		$checkbox_targets                          = [
		...
		$label_text_decoration_font_properties      = LabelTextDecorationFontProperties::CSS_PROPERTIES;
								...
								'advancedStyles'           => [
									...
									[
										...
										'props'         => [
											...
											'attr'                => $attrs['module']['advanced']['showLabels'] ?? [],
											'declarationFunction' => [ self::class, 'show_contact_form_labels_declaration' ],
										],
									],
									[
										'componentName' => 'divi/common',
										'props'         => [
											...
											'attr'                => $attrs['module']['advanced']['showLabels'] ?? [],
											'declarationFunction' => [ self::class, 'hide_contact_field_options_title_declaration' ],
										],
									],
								],
							],
	...
	/**
	 * Viewport rank from customizable breakpoint settings (smaller = narrower).
	 *
	 * @param string $breakpoint Breakpoint name.
	 *
	 * @return int|null Breakpoint order, or null when unknown.
	 */
	private static function show_labels_breakpoint_order( string $breakpoint ): ?int {
		$items = Breakpoint::get_settings_values()['items'] ?? [];

		if ( ! isset( $items[ $breakpoint ]['order'] ) ) {
			return null;
		}

		return (int) $items[ $breakpoint ]['order'];
	}

	/**
	 * Whether an existing Show Labels on would still apply at this breakpoint.
	 *
	 * Default-off must not emit CSS so global hide and existing custom CSS stay in charge.
	 * Base-breakpoint on is unscoped and covers every viewport, so off at smaller max-width
	 * breakpoints and larger min-width breakpoints still needs an override. A smaller max-width
	 * on leaks down (style at-rules use max-width only below the base). A larger min-width on
	 * does not leak up: when a larger breakpoint is enabled, that query is a ranged min+max,
	 * so ultraWide off after widescreen-only on stays silent.
	 *
	 * Rank comes from customizable breakpoint settings (`order` + base device), not a hardcoded map.
	 *
	 * @param array  $attr       Full Show Labels breakpoint attr.
	 * @param string $breakpoint Current breakpoint name.
	 *
	 * @return bool True when on CSS from another breakpoint applies here.
	 */
	private static function show_labels_has_on_css_that_applies_at_breakpoint( array $attr, string $breakpoint ): bool {
		$current_order = self::show_labels_breakpoint_order( $breakpoint );
		...
		$base_breakpoint_order = self::show_labels_breakpoint_order( $base_breakpoint );

		if ( null === $base_breakpoint_order ) {
			return false;
		}

		foreach ( $attr as $name => $states ) {
			if ( $name === $breakpoint || 'on' !== ( $states['value'] ?? null ) ) {
				continue;
			}

			// Unscoped base-breakpoint CSS applies at every viewport.
			if ( $name === $base_breakpoint ) {
				return true;
			}

			$on_order = self::show_labels_breakpoint_order( (string) $name );

			if ( null === $on_order ) {
				continue;
			}

			// Max-width on leaks to smaller viewports (higher-index / lower-order than base).
			// Min-width on does not leak to larger viewports: get_at_rules() ranges those queries.
			if ( $on_order < $base_breakpoint_order && $current_order < $base_breakpoint_order && $current_order <= $on_order ) {
				return true;
			}
		}
	...
	/**
	 * Show or hide field labels per breakpoint from Show Labels.
	 *
	 * Overrides the global `.et_pb_contact_form_label { display: none; }` rule under this form only.
	 * Default off emits nothing so custom CSS can still reveal labels.
	 *
	 * @since ??
	 *
	 * @param array $params {
	 ...
	 *
	 *     @type mixed  $attrValue  Show Labels attribute value for the current breakpoint/state.
	 *     @type array  $attr       Full Show Labels breakpoint attr.
	 *     @type string $breakpoint Current breakpoint name.
	 * }
	 *
	 * @return string CSS declarations.
	 */
	public static function show_contact_form_labels_declaration( array $params ): string {
		$attr_value = $params['attrValue'] ?? 'off';
		$attr       = $params['attr'] ?? [];
		$breakpoint = $params['breakpoint'] ?? 'desktop';

		if ( 'on' !== $attr_value && ! self::show_labels_has_on_css_that_applies_at_breakpoint( $attr, $breakpoint ) ) {
			return '';
		}
		...
		// Explicit on/off so other breakpoints can override this one.
		$style_declarations->add( 'display', 'on' === $attr_value ? 'block' : 'none' );

		return $style_declarations->value();
	}

	/**
	 * Show or hide checkbox/radio options titles per breakpoint from Show Labels.
	 * When labels are shown, hide options titles to avoid duplicate titles.
	 * Default off emits nothing; off after another on restores titles via CSS.
	 *
	 * @since ??
	 *
	 * @param array $params {
	 ...
	 * @return string CSS declarations.
	 */
	public static function hide_contact_field_options_title_declaration( array $params ): string {
		$attr_value = $params['attrValue'] ?? 'off';
		$attr       = $params['attr'] ?? [];
		$breakpoint = $params['breakpoint'] ?? 'desktop';
		...
		// Explicit on/off so other breakpoints can override this one without JS.
		$style_declarations->add( 'display', 'on' === $attr_value ? 'none' : 'block' );
