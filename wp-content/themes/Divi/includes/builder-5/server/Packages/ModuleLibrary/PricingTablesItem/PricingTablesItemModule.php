...
class PricingTablesItemModule implements DependencyInterface {
	...
	public static function render_callback( $attrs, $child_modules_content, $block, $elements, $default_printed_style_attrs ) {
		...
		$parent_button_attr              = $parent_attrs['button']['decoration']['button']['desktop']['value'] ?? [];
		$filtered_button_decoration_attr = self::filter_button_decoration_attrs(
			$attrs['button']['decoration'] ?? [],
			$parent_button_attr
		);
		$button_element_attr             = array_merge(
			$attrs['button'] ?? [],
			[
				'decoration' => $filtered_button_decoration_attr,
			]
		);
		...
		$button = $elements->render(
			[
				'attrName'    => 'button',
				'elementAttr' => $button_element_attr,
			]
		);
	...
	public static function filter_button_decoration_attrs( array $decoration_attrs, array $parent_button_attr ): array {
		...
		$parent_icon_settings = $parent_button_attr['icon']['settings'] ?? [];
		$attr_breakpoints     = ! empty( $button_attr ) ? array_keys( $button_attr ) : [ 'desktop' ];

		// Iterate through each attrState for every attrBreakpoint in button attrs.
		foreach ( $attr_breakpoints as $attr_breakpoint ) {
			$attr_state_values = $button_attr[ $attr_breakpoint ] ?? [ 'value' => [] ];
			$attr_states       = ! empty( $attr_state_values ) ? array_keys( $attr_state_values ) : [ 'value' ];

			foreach ( $attr_states as $attr_state ) {
				...
				$icon_enable   = $button_attrs[ $attr_breakpoint ][ $attr_state ]['icon']['enable'] ?? '';
				...
				// Inherit parent icon settings when child icon settings are not explicitly set.
				if ( ! empty( $parent_icon_settings ) && empty( $icon_settings ) && 'off' !== $icon_enable ) {
					$button_attrs[ $attr_breakpoint ][ $attr_state ]['icon']['settings'] = $parent_icon_settings;
				}
			}
		}
