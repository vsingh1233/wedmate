...
class GalleryModule implements DependencyInterface {
	...
	/**
	 * Filters the item.decoration attributes.
	 *
	 * This function is equivalent of JS function filterItemDecorationAttrs located in
	 * visual-builder/packages/module-library/src/components/gallery/attrs-filter/filter-item-decoration-attrs/index.ts.
	 *
	 * @since ??
	 *
	 * @param array $decoration_attrs The decoration attributes to be filtered.
	 ...
	 * @return array The filtered decoration attributes.
	 */
	public static function filter_item_decoration_attrs( array $decoration_attrs, array $attrs ): array {
		// Attribute `module.advanced.fullwidth` is desktop only.
		$is_slider = $attrs['module']['advanced']['fullwidth']['desktop']['value'] ?? null;

		// In slider mode, item border is applied on the module wrapper via moduleAdvancedStyles.
		if ( 'on' === $is_slider ) {
			$decoration_attrs = array_merge(
				$decoration_attrs,
				[
	...
	public static function icon_font_style_declaration( array $params ): string {
		...
		if ( ! empty( $hover_icon['type'] ) ) {
			$font_family = IconFontUtils::get_icon_font_family( $hover_icon['type'] ?? null );
	 ...
	 *
	 * Applies border styles on the module wrapper only when slider mode is active and border attribute exists.
	...
	public static function border_style_declaration( array $params ) {
		...
		// Apply border on the module wrapper only when slider mode is active and border attr exists.
	...
	/**
	 * Returns whether a border attribute value has any non-zero corner radius.
	 *
	 * @since ??
	 *
	 * @param array $attr_value Border attribute value.
	 *
	 * @return bool True when any corner radius is non-zero.
	 */
	private static function border_has_non_zero_radius( array $attr_value ): bool {
		...
		foreach ( $corners as $corner ) {
			...
			$resolved = ImageModule::resolve_border_length_for_decoration_flag( $radius[ $corner ] );
			...
			if ( is_string( $resolved ) && ! ImageModule::border_decoration_length_is_explicit_zero( $resolved ) ) {
				return true;
			}
		}
	...
	/**
	 * Returns inner border-radius for one corner, subtracting border width while preserving units.
	 *
	 * @since ??
	 *
	 * @param string $radius_value Corner border-radius value.
	 * @param string $border_width Border width value.
	 *
	 * @return string Inner border-radius value for the corner.
	 */
	private static function get_inner_border_radius_corner_value( string $radius_value, string $border_width ): string {
		$radius_value = trim( $radius_value );
		$border_width = trim( $border_width ) ?: '0px';

		if ( preg_match( '/^(clamp|min|max|calc)\s*\(/', $radius_value ) ) {
			return "max(0px, calc({$radius_value} - {$border_width}))";
		}

		if ( preg_match( '/^(-?[\d.]+)(.*)$/', $radius_value, $radius_match ) ) {
			$radius_num  = floatval( $radius_match[1] );
			$radius_unit = $radius_match[2] ?: 'px';

			if ( preg_match( '/^(-?[\d.]+)(.*)$/', $border_width, $border_match ) ) {
				$border_num  = floatval( $border_match[1] );
				$border_unit = $border_match[2] ?: 'px';

				if ( 'px' === $radius_unit && ( 'px' === $border_unit || '' === $border_unit ) ) {
					return max( 0, $radius_num - $border_num ) . 'px';
				}
			}
	...
	/**
	 * Returns a side width, falling back to the synced `all` width.
	 *
	 * @since ??
	 *
	 * @param array  $styles Border styles.
	 * @param string $side   Border edge.
	 *
	 * @return string Border width for the side.
	 */
	private static function get_side_border_width( array $styles, string $side ): string {
		$side_width = trim( (string) ( $styles[ $side ]['width'] ?? '' ) );

		if ( '' !== $side_width ) {
			return $side_width;
		}

		$all_width = trim( (string) ( $styles['all']['width'] ?? '' ) );

		return '' !== $all_width ? $all_width : '0px';
	}

	/**
	 * Returns the larger of two CSS length values for inner-clip inset.
	 *
	 * @since ??
	 *
	 * @param string $first  First width value.
	 * @param string $second Second width value.
	 *
	 * @return string Maximum width value.
	 */
	private static function get_max_border_width( string $first, string $second ): string {
		$trimmed_first  = trim( $first ) ?: '0px';
		$trimmed_second = trim( $second ) ?: '0px';

		if ( $trimmed_first === $trimmed_second ) {
			return $trimmed_first;
		}

		$first_is_px  = preg_match( '/^(-?[\d.]+)(px)?$/', $trimmed_first, $first_match );
		$second_is_px = preg_match( '/^(-?[\d.]+)(px)?$/', $trimmed_second, $second_match );

		if ( $first_is_px && $second_is_px ) {
			return max( floatval( $first_match[1] ), floatval( $second_match[1] ) ) . 'px';
		}

		return "max({$trimmed_first}, {$trimmed_second})";
	}

	/**
	 * Returns the border width to subtract for one radius corner.
	 *
	 * @since ??
	 *
	 * @param array  $styles Border styles.
	 * @param string $corner Radius corner name.
	 *
	 * @return string Width to subtract from the corner radius.
	 */
	private static function get_corner_border_width( array $styles, string $corner ): string {
		$corner_adjacent_edges = [
			'topLeft'     => [ 'top', 'left' ],
			'topRight'    => [ 'top', 'right' ],
		...
		if ( ! isset( $corner_adjacent_edges[ $corner ] ) ) {
			$all_width = trim( (string) ( $styles['all']['width'] ?? '' ) );

			return '' !== $all_width ? $all_width : '0px';
		}

		$first_edge  = $corner_adjacent_edges[ $corner ][0];
		$second_edge = $corner_adjacent_edges[ $corner ][1];

		return self::get_max_border_width(
			self::get_side_border_width( $styles, $first_edge ),
			self::get_side_border_width( $styles, $second_edge )
		);
	}

	/**
	 * Returns inner border-radius values adjusted for border width on the module wrapper.
	 *
	 * @since ??
	 *
	 * @param array $attr_value Border attribute value.
	 *
	 * @return array Radius-only attr value for inner clipping.
	 */
	private static function get_inner_border_radius_attr_value( array $attr_value ): array {
		...
		$styles       = is_array( $attr_value['styles'] ?? null ) ? $attr_value['styles'] : [];
		$inner_radius = [];

		foreach ( $radius as $corner => $value ) {
			if ( 'sync' === $corner ) {
				$inner_radius[ $corner ] = $value;
				continue;
			}

			$inner_radius[ $corner ] = self::get_inner_border_radius_corner_value(
				(string) $value,
				self::get_corner_border_width( $styles, (string) $corner )
			);
		}
	...
	/**
	 * Slider gallery image clip style declaration.
	 *
	 * Applies inner border-radius and overflow:hidden on `.et_pb_gallery_image` in slider mode so image
	 * and hover overlay clip to rounded corners without duplicating border width on the item.
	 *
	 * Overflow uses !important so VB module-hover (`et_vb_options_visible`) cannot force overflow:visible
	 * on the clipping container. Clip stays on the image so the module wrapper can keep box-shadow.
	 *
	 * This function is equivalent of JS function sliderGalleryImageClipStyleDeclaration located in
	 * visual-builder/packages/module-library/src/components/gallery/style-declarations/border/index.ts.
	 *
	 * @since ??
	 *
	 ...
	 *
	 * @return string Combined border-radius and overflow CSS declaration.
	 */
	public static function slider_gallery_image_clip_style_declaration( array $params ) {
		$attr_value = $params['attrValue'] ?? [];

		if ( ! self::border_has_non_zero_radius( $attr_value ) ) {
			return '';
		}

		$radius_attr_value = self::get_inner_border_radius_attr_value( $attr_value );

		$radius_declaration = '';

		if ( ! empty( $radius_attr_value ) ) {
			$radius_declaration = Border::style_declaration(
				array_merge(
					$params,
					[
						'attrValue'  => $radius_attr_value,
						'returnType' => 'string',
					]
				)
			);
		...
		$overflow_declarations = new StyleDeclarations(
			[
				'returnType' => 'string',
				'important'  => true,
			]
		...
		$overflow_declarations->add( 'overflow', 'hidden' );

		$overflow_declaration = $overflow_declarations->value();
	...
	public static function module_styles( $args ) {
		...
		$item_advanced_styles = $is_slider ? [] : [
			[
		...
		$module_slider_image_clip_styles = $is_slider ? [
			[
				'componentName' => 'divi/common',
				'props'         => [
					'selector'            => $args['orderClass'] . '.et_pb_gallery .et_pb_gallery_image',
					'attr'                => $attrs['item']['decoration']['border'] ?? [],
					'declarationFunction' => [ self::class, 'slider_gallery_image_clip_style_declaration' ],
				],
			],
			[
				'componentName' => 'divi/common',
		...
		$module_advanced_styles = array_merge( $module_advanced_styles, $module_slider_image_clip_styles );
				...
				'styles'        => [
					...
					$elements->style(
						[
							...
							'styleProps' => [
								'attrsFilter'      => function ( $decoration_attrs ) use ( $attrs ) {
									return GalleryModule::filter_item_decoration_attrs( $decoration_attrs, $attrs );
								},
								'advancedStyles'   => $item_advanced_styles,
							],
						]
					),
					// image.
