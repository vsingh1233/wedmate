...
class ContactForm7Module implements DependencyInterface {
	...
	/**
	 * Minimal responsive attribute so divi/common emits desktop caption-box rules once.
	 *
	 * @since ??
	 *
	 * @return array<string, mixed>
	 */
	public static function label_caption_box_attr(): array {
		return [
			'desktop' => [
				'value' => 'on',
			],
	...
	/**
	 * Field-caption labels that are a direct form child or nested one level (p / div wrappers).
	 *
	 * Nested choice labels (`.wpcf7-list-item label`) are deeper and must not match.
	 *
	 * @since ??
	 *
	 ...
	 */
	public static function label_caption_selectors( string $order_class ): string {
		return implode(
			', ',
			[
				"{$order_class} form > label",
				"{$order_class} form > p > label",
				"{$order_class} form > div > label",
			]
		);
	}

	/**
	 * Block formatting context so Label Text Alignment (`text-align`) can take effect on CF7 captions.
	 *
	 * @since ??
	 *
	 * @param array<string, mixed> $params Style declaration parameters.
	 *
	 * @return string CSS declarations.
	 */
	public static function label_caption_box_declaration( array $params ): string {
		$attr_value = $params['attrValue'] ?? '';

		if ( ! is_string( $attr_value ) || 'on' !== $attr_value ) {
			return '';
		}
	 ...
	 *
	 * Mirrors Button group sizing width to max-width so custom submit width
	 * is not clipped by Button-module `max-width: max-content`.
	 *
	 * @since ??
	 *
	 * @param array<string, mixed> $params Style declaration arguments.
	 *
							...
							'styleProps' => [
								...
								'advancedStyles'           => [
									[
										...
										'props'         => [
											'selector'            => self::label_caption_selectors( $order_class ),
											'attr'                => self::label_caption_box_attr(),
											'declarationFunction' => [ self::class, 'label_caption_box_declaration' ],
										],
									],
								],
							],
									...
									[
										...
										'props'         => [
											'selector' => $is_custom_post_type
												? "body.et-db #page-container #et-boc .et-l .et_pb_section {$order_class} .et_pb_button.wpcf7-submit"
												: "body #page-container .et_pb_section {$order_class} .et_pb_button.wpcf7-submit",
