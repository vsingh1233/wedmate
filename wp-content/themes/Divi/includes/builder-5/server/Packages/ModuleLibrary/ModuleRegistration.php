...
class ModuleRegistration {
	...
	/**
	 * Nested depth for suppressing ScriptData/Fonts while still rendering styles.
	 *
	 * Used when Conditional Display discards HTML but still runs the render callback
	 * so Dynamic Assets can collect styles (D4 parity). Nested non-displayable modules
	 * increment/decrement this depth so a child finally block cannot clear an outer suppress.
	 *
	 * @var int
	 */
	private static $_suppress_runtime_assets_depth = 0;
	...
	public static function register_module( string $metadata_folder, array $args = [] ): ?WP_Block_Type {
		...
		if ( isset( $register_args['render_callback'] ) ) {
			...
			$register_args['render_callback'] = function ( $block_attributes, $content, WP_Block $block ) use ( $loop_wrapped_render_callback, $metadata ) {
				...
				$preserve_gradient_stop_integrity = static function ( array $deduped_attrs, array $source_attrs ): array {
					$deduped_background = $deduped_attrs['module']['decoration']['background'] ?? null;
					$source_background  = $source_attrs['module']['decoration']['background'] ?? null;

					if ( ! is_array( $deduped_background ) || ! is_array( $source_background ) ) {
						return $deduped_attrs;
					}

					foreach ( $deduped_background as $breakpoint => $states ) {
						if ( ! is_array( $states ) ) {
							continue;
						}
						...
						foreach ( $states as $state => $row ) {
							...
							$deduped_stops = $row['gradient']['stops'] ?? null;
							$source_stops  = $source_background[ $breakpoint ][ $state ]['gradient']['stops'] ?? null;

							if ( ! is_array( $deduped_stops ) || ! is_array( $source_stops ) ) {
								continue;
							}

							$has_missing_stop_rows = count( $deduped_stops ) < count( $source_stops );
							$has_surviving_stops   = ! empty( $deduped_stops );

							// Restore gradient stops atomically when dedupe removed matching stop rows.
							if ( $has_missing_stop_rows && $has_surviving_stops ) {
								$deduped_attrs['module']['decoration']['background'][ $breakpoint ][ $state ]['gradient']['stops'] = $source_stops;
								$deduped_stops = $source_stops;
							}

							foreach ( $deduped_stops as $stop_index => $deduped_stop ) {
								$source_stop = $source_stops[ $stop_index ] ?? null;

								if ( ! is_array( $deduped_stop ) || ! is_array( $source_stop ) ) {
									continue;
								}

								$has_deduped_color    = isset( $deduped_stop['color'] ) && '' !== (string) $deduped_stop['color'];
								$has_deduped_position = isset( $deduped_stop['position'] ) && '' !== (string) $deduped_stop['position'];
								$has_source_color     = isset( $source_stop['color'] ) && '' !== (string) $source_stop['color'];
								$has_source_position  = isset( $source_stop['position'] ) && '' !== (string) $source_stop['position'];

								// Preserve stop objects atomically when dedupe strips only one key.
								if (
									$has_source_color &&
									$has_source_position &&
									(
										( $has_deduped_color && ! $has_deduped_position ) ||
										( ! $has_deduped_color && $has_deduped_position )
									)
								) {
									$deduped_attrs['module']['decoration']['background'][ $breakpoint ][ $state ]['gradient']['stops'][ $stop_index ] = $source_stop;
								}
							}
						}
					}
				...
				if ( $should_strip_preset_dupes && ! empty( $block_attributes ) ) {
					$block_attributes_before_dedupe = $block_attributes;
					$block_attributes = ModuleUtils::remove_matching_values( $block_attributes, $preset_cleanup_reference );
					$block_attributes = $preserve_gradient_stop_integrity( $block_attributes, $block_attributes_before_dedupe );
				}
				...
				if ( $should_strip_preset_dupes && ! empty( $module_style_attrs ) ) {
					$module_style_attrs_before_dedupe = $module_style_attrs;
					$module_style_attrs = ModuleUtils::remove_matching_values( $module_style_attrs, $preset_cleanup_reference );
					$module_style_attrs = $preserve_gradient_stop_integrity( $module_style_attrs, $module_style_attrs_before_dedupe );
				}
				...
				// Runtime ScriptData/Fonts suppress for Conditional Display is started earlier on
				// `render_block_data` (see ConditionsRenderer) so container modules gate descendants
				// before WordPress renders inner blocks into $content.
	...
	/**
	 * Resolve module attrs for FE renderability checks (Conditional Display / full disabledOn).
	 *
	 * Early `render_block_data` suppress and Dynamic Assets font-extraction gating see raw saved
	 * attrs only. Preset-sourced Conditional Display / `disabledOn` live in module/group
	 * `renderAttrs`, so those surfaces must merge the same runtime layers the render callback
	 * uses before deciding whether a block (and its descendants) can render.
	 *
	 * Merge order matches the render-callback runtime attrs path:
	 * defaults → stacked module renderAttrs → group renderAttrs → raw attrs (raw wins).
	 *
	 * @since ??
	 *
	 * @param string $block_name Module block name (for example `divi/blurb`).
	 * @param array  $raw_attrs  Raw saved block attributes.
	 *
	 * @return array Resolved attributes for renderability evaluation.
	 */
	public static function resolve_module_attrs_for_renderability( string $block_name, array $raw_attrs ): array {
		$default_attrs = self::get_default_attrs( $block_name );
		...
		$merged_group_preset_attrs = GlobalPreset::merge_selected_group_presets_by_priority( $group_presets );
		$group_render_attrs        = is_array( $merged_group_preset_attrs['renderAttrs'] ?? null )
			? $merged_group_preset_attrs['renderAttrs']
			: [];
	...
	/**
	 * Public wrapper for fully-disabled breakpoint gating.
	 *
	 * Used by supporting surfaces such as Dynamic Assets font extraction so they can
	 * skip modules that would never render on the frontend.
	 *
	 * @since ??
	 *
	 * @param array $module_attrs Module attributes including decoration values.
	 *
	 * @return bool True when disabled on every enabled breakpoint.
	 */
	public static function is_disabled_on_all_breakpoints( array $module_attrs ): bool {
		return self::_is_disabled_on_all_breakpoints( $module_attrs );
	}
	...
	/**
	 * Public wrapper for interaction-target detection.
	 *
	 * Used by supporting surfaces such as Dynamic Assets font extraction so hidden
	 * interaction targets keep their expected DOM presence behavior.
	 *
	 * @since ??
	 *
	 * @param array $module_attrs Module attributes including decoration values.
	 ...
	 * @return bool True when module is targeted by an interaction.
	 */
	public static function is_interaction_target( array $module_attrs ): bool {
		return self::_is_interaction_target( $module_attrs );
	}
	...
	/**
	 * Begin suppressing ScriptData/Fonts registration for the current render.
	 *
	 * Styles continue to register. Nesting-safe via depth counter.
	 * Ownership of when to begin/end for Conditional Display lives in
	 * ConditionsRenderer (`render_block_data` / `render_block`) so containers
	 * suppress descendants before WordPress renders inner blocks.
	 *
	 * @since ??
	 *
	 * @return void
	 */
	public static function begin_suppress_runtime_assets(): void {
		self::$_suppress_runtime_assets_depth++;
	}

	/**
	 * End suppressing ScriptData/Fonts registration for the current render.
	 *
	 * @since ??
	 *
	 * @return void
	 */
	public static function end_suppress_runtime_assets(): void {
		if ( self::$_suppress_runtime_assets_depth > 0 ) {
			self::$_suppress_runtime_assets_depth--;
		}
	}

	/**
	 * Whether ScriptData/Fonts registration should be skipped for the current render.
	 *
	 * @since ??
	 *
	 * @return bool
	 */
	public static function should_suppress_runtime_assets(): bool {
		return self::$_suppress_runtime_assets_depth > 0;
	}
}
