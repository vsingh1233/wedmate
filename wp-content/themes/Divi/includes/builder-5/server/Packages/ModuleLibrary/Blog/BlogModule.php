...
class BlogModule implements DependencyInterface {
	...
	public static function render_callback( $attrs, $content, $block, $elements ) {
		...
		// Resolve Blog page number with D4 parity (#42889 / EB-BQ-05):
		// archives use `paged`; the front page uses `page`. When `et_blog` is present,
		// `et_builder_filter_main_query_paged_for_blog_module` stashes the value in
		// `$__et_blog_module_paged` and zeros the main query to avoid clashes.
		$et_blog_module_page = absint( is_front_page() ? get_query_var( 'page' ) : get_query_var( 'paged' ) );

		if ( $__et_blog_module_paged > 1 ) {
			$et_blog_module_page = (int) $__et_blog_module_paged;
		}
		...
		$query_args = [
			...
			'perm'           => 'readable',
			'post_type'      => $post_type,
			'paged'          => $et_blog_module_page,
		];

		// Keep global `$paged` aligned for `next_posts_link` / `previous_posts_link` (D4 parity).
		...
		if ( $wp_query->have_posts() ) {
			...
			// Prepend stickies on page 1 only; still skip their IDs in the main loop below (#51635).
			if ( ! empty( $sticky_posts ) && self::is_sticky_prepend_page() ) {
				$sticky_args = [
					'post_type'      => $post_type,
				...
				// Posts for Current Page: scope stickies to the viewed archive term so stickies.
				// from other categories/tags do not leak across archive TB bodies (#49675 / EB-BQ-01).
				if ( 'on' === $use_current_loop && ! is_singular() ) {
					$sticky_args = ModuleUtils::handle_archive_context_for_query( $sticky_args );
				}
	...
	/**
	 * Determines whether the extra sticky query should prepend posts for this listing page.
	 *
	 * Native WordPress sticky promotion only runs on page 1. D5 Blog uses an extra
	 * query when `is_home` promotion does not apply (#37260). That prepend is gated
	 * to page 1 so stickies do not repeat on later pages (#51635).
	 *
	 * @since ??
	 *
	 * @param int|null $paged Optional explicit page number from REST. When null, the
	 *                        page is resolved from `$__et_blog_module_paged`, then
	 *                        from the `page` or `paged` query var.
	 *
	 * @return bool True when the extra sticky query should run.
	 */
	public static function is_sticky_prepend_page( ?int $paged = null ): bool {
		if ( null === $paged ) {
			global $__et_blog_module_paged;

			if ( isset( $__et_blog_module_paged ) && 1 < $__et_blog_module_paged ) {
				$paged = absint( $__et_blog_module_paged );
			} else {
				$query_var = true === is_front_page() ? 'page' : 'paged';
				$paged     = absint( get_query_var( $query_var ) );
			}
		}

		return 1 === max( 1, $paged );
	}
	 ...
	 *
	 * On category/tag/tax archives with `/%category%/%postname%/` (and similar) structures, pretty
	 * `/page/N/` segments can 404. Normalize those archive pagination links to an explicit `paged`
	 * query arg while preserving the `et_blog` clash flag (#42889 / EB-BQ-05).
	 *
	 * @since ??
	 *
	...
	public static function filter_pagination_url( $result ) {
		if ( ( is_category() || is_tag() || is_tax() ) && preg_match( '#/page/(\d+)/?#', $result, $matches ) ) {
			$paged = max( 1, (int) $matches[1] );
			$base  = preg_replace( '#/page/\d+/?#', '/', $result );
			$base  = remove_query_arg( [ 'paged', 'page', 'et_blog' ], $base );
