...
class BlogController extends RESTController {
	...
	public static function index( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( $query->have_posts() ) {
			// Display sticky posts first on page 1 only (#51635).
			if ( ! empty( $sticky_posts ) && BlogModule::is_sticky_prepend_page( absint( $args['paged'] ) ) ) {
				$sticky_args = [
					'post_type'      => $args['post_type'],
				...
				// Posts for Current Page: mirror main-query archive term onto sticky query (#49675).
				if ( 'on' === $args['use_current_loop'] && $should_apply_archive_term_context && $args['archive_term_id'] > 0 && empty( $sticky_args['cat'] ) && empty( $sticky_args['tax_query'] ) ) {
					$sticky_term = get_term( $args['archive_term_id'] );

					if ( ! is_wp_error( $sticky_term ) && $sticky_term instanceof \WP_Term ) {
						if ( 'category' === $sticky_term->taxonomy ) {
							$sticky_args['cat'] = $sticky_term->term_id;
						} else {
							$sticky_args['tax_query'] = [
								[
									'taxonomy' => $sticky_term->taxonomy,
									'field'    => 'term_id',
									'terms'    => $sticky_term->term_id,
								],
							];
						}
					}
