...
class LoginModule implements DependencyInterface {
	...
	public static function icon_style_declaration( array $params ): string {
		...
		if ( ! empty( $icon_attr['settings']['type'] ) ) {
			$font_family = Utils::get_icon_font_family( $icon_attr['settings']['type'] ?? null );
		...
		Style::add(
			[
				...
				'styles'        => [
					...
					CommonStyle::style(
						[
							'selector'               => "{$args['orderClass']} .et_pb_login_logout_link",
							'attr'                   => [
								'desktop' => [
									'value' => true,
								],
	...
	public static function render_callback( array $attrs, string $child_modules_content, WP_Block $block, ModuleElements $elements ): string {
		...
		$has_content_attr      = ModuleUtils::has_value( $attrs['content']['innerContent'] ?? [] );
		$is_logged_in_context  = is_user_logged_in() && ! is_customize_preview() && ! is_et_pb_preview();
		$is_render_content     = $has_content_attr || $is_logged_in_context;
		$current_page_redirect = $attrs['module']['advanced']['currentPageRedirect']['desktop']['value'] ?? 'off';
		$content_element_html  = '';

		if ( $is_render_title || $is_render_content ) {
			if ( $is_logged_in_context && ! $has_content_attr ) {
				$content_element_html = HTMLUtility::render(
					[
						'tag'               => 'div',
						'attributes'        => [
							'class' => 'et_pb_newsletter_description_content',
						],
						'children'          => LoginModule::render_logged_in_as_content( '', $current_page_redirect ),
						'childrenSanitizer' => 'et_core_esc_previously',
					]
				);
			} else {
				$content_element_html = $elements->render(
					[
						'selector'        => '{{selector}} .et_pb_newsletter_description_content',
						'attrName'        => 'content',
						'allowEmptyValue' => $is_logged_in_context, // Allow empty body content so logged-in status markup can still render.
