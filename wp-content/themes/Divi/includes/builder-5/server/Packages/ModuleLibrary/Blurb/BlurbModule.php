...
class BlurbModule implements DependencyInterface {
	...
	public static function icon_style_declaration( array $params ): string {
		...
		if ( isset( $icon_attr['type'] ) ) {
			$font_family = Utils::get_icon_font_family( $icon_attr['type'] ?? null );
	 ...
	 */
	private static function _align_self_to_alignment( string $align_self ): string {
		switch ( $align_self ) {
			case 'flex-start':
				return 'left';
	...
	/**
	 * Whether a Blurb image width uses a relative unit.
	 *
	 * Pixel widths stay on the SVG sizing root. Relative units (%, em, rem, etc.)
	 * target the wrap so they do not compound with a leftover container width.
	 *
	 * @since ??
	 *
	 * @param mixed $width_value The width value.
	 *
	 * @return bool
	 */
	private static function _is_relative_image_width( $width_value ): bool {
		return is_string( $width_value ) && '' !== $width_value && 'auto' !== $width_value && ! str_contains( $width_value, 'px' );
	}

	/**
	 * Resolve a breakpoint/state value for Blurb image width helpers.
	 *
	 * Uses enabled breakpoint names and the actual base breakpoint so frontend
	 * inheritance matches Visual Builder app state.
	 *
	 * @since ??
	 *
	 * @param array  $attr       Breakpoint/state attr tree.
	 * @param string $breakpoint The breakpoint name.
	 * @param string $state      The state name.
	 * @param string $mode       Attribute inheritance mode.
	 *
	 * @return array
	 */
	private static function _get_breakpoint_state_value( array $attr, string $breakpoint, string $state, string $mode = 'getAndInheritAll' ): array {
		$inherited_value = ModuleUtils::use_attr_value(
			[
				'attr'         => $attr,
		...
		return is_array( $inherited_value ) ? $inherited_value : [];
	}

	/**
	 * Split top-placement SVG width attrs onto the container root vs the wrap.
	 *
	 * CommonStyle uses one selector per attr tree. A module-wide relative-width flag
	 * would send every breakpoint to the wrap, including unset SVG states that must
	 * keep `width: 100%` on `.et_pb_main_blurb_image`. Relative leaves stay on the wrap
	 * and rewrite the same breakpoint/state on the container to `100%` so leftover px
	 * does not compound. Explicit pixel/auto leaves reset the wrap to `100%` so preset
	 * or inherited wrap widths cannot override the container sizing root.
	 *
	 * @since ??
	 *
	 * @param array $image_width_attr Merged innerContent + sizing attr.
	 *
	 * @return array{container: array, wrap: array}
	 */
	private static function _split_svg_image_width_attrs( array $image_width_attr ): array {
		$container_attr = $image_width_attr;
		$wrap_attr      = [];
		$desktop_src    = $image_width_attr['desktop']['value']['src'] ?? '';

		foreach ( $image_width_attr as $breakpoint => $breakpoint_values ) {
			...
			foreach ( $breakpoint_values as $state => $state_value ) {
				...
				if ( ! self::_is_relative_image_width( $width_value ) ) {
					$resolved_state_value  = self::_get_breakpoint_state_value( $image_width_attr, $breakpoint, $state );
					$inherited_state_value = self::_get_breakpoint_state_value( $image_width_attr, $breakpoint, $state, 'inheritAll' );
					$inherited_width_value = $inherited_state_value['width'] ?? '';

					if (
						is_array( $state_value )
						&& is_string( $width_value )
						&& '' !== $width_value
						&& empty( $state_value['alignSelf'] )
						&& ! empty( $resolved_state_value['alignSelf'] )
					) {
						$container_attr[ $breakpoint ][ $state ]['alignSelf'] = $resolved_state_value['alignSelf'];
					}
					...
					$wrap_state = $state_value;

					if ( self::_is_relative_image_width( $inherited_width_value ) ) {
						if ( empty( $wrap_state['src'] ) && is_string( $desktop_src ) && '' !== $desktop_src ) {
							$wrap_state['src'] = $desktop_src;
						}
					} else {
						unset( $wrap_state['alignSelf'], $wrap_state['src'] );
					}

					$wrap_state['width'] = '100%';

					$wrap_attr[ $breakpoint ][ $state ] = $wrap_state;
					continue;
				}
				...
				$container_state = is_array( $container_attr[ $breakpoint ][ $state ] ?? null )
					? $container_attr[ $breakpoint ][ $state ]
					: [];
				unset( $container_state['alignSelf'] );
				$container_state['width']                = '100%';
				$container_attr[ $breakpoint ][ $state ] = $container_state;
			}
		}
	...
	/**
	 * Split SVG sizing alignment attrs onto the container root vs the wrap.
	 *
	 * Relative-width leaves align the wrap. Pixel and unset-width leaves align the
	 * container so mixed breakpoints keep the same selector split as image width.
	 *
	 * @since ??
	 *
	 * @param array $image_sizing_attr Image icon sizing attr.
	 *
	 * @return array{container: array, wrap: array}
	 */
	private static function _split_svg_image_sizing_alignment_attrs( array $image_sizing_attr ): array {
		$container_attr = [];
		$wrap_attr      = [];
		...
		foreach ( $image_sizing_attr as $breakpoint => $breakpoint_values ) {
			...
			foreach ( $breakpoint_values as $state => $state_value ) {
				$inherited_state_value = self::_get_breakpoint_state_value( $image_sizing_attr, $breakpoint, $state );
				$width_value           = $inherited_state_value['width'] ?? '';
				$alignment_state       = $state_value;

				if ( empty( $alignment_state['alignSelf'] ) && ! empty( $inherited_state_value['alignSelf'] ) ) {
					$alignment_state['alignSelf'] = $inherited_state_value['alignSelf'];
				}

				if ( self::_is_relative_image_width( $width_value ) ) {
					$wrap_attr[ $breakpoint ][ $state ] = $alignment_state;
					continue;
				}

				$container_attr[ $breakpoint ][ $state ] = $alignment_state;
			}
		}
	...
	public static function icon_sizing_alignment_style_declaration( array $params ): string {
		...
		$alignment = self::_align_self_to_alignment( $align_self );
	...
	/**
	 * Sets the image width, max-width, and min-width style declarations for the Blurb module.
	 *
	 * Max-width and min-width are emitted on the image wrapper selector; default sizing strips them whenever
	 * this bypass runs (same as width on preset/icon paths; module + image strips max/min in attrsFilter).
	 *
	 * This function adds style declarations to the Blurb module's image based on the
	 * provided parameters. It uses the value (breakpoint > state > value) of the module
	 * attribute. The CSS style declaration is returned as a string. Callers must attach
	 * max-width/min-width to the image wrap selector only; width may use the dual SVG selector.
	 *
	 * @since ??
	 *
	 * @param array $params {
	 *     An array of arguments.
	 *
	 *     @type array $attrValue The value (breakpoint > state > value) of the module attribute, including optional width, maxWidth, minWidth.
	 * }
	 *
	 * @return string The CSS style declarations for image width, max-width, and min-width.
	 *
	 ...
	 * // Result: "width: 500px; max-width: 100%; min-width: 10px;"
	 * ```
	 */
	public static function image_width_style_declaration( array $params ): string {
		...
		$width_value = $attr_value['width'] ?? '';
		$src         = $attr_value['src'] ?? '';
		$align_self  = $attr_value['alignSelf'] ?? '';

		// Preserve width and alignment inheritance when src changes on hover or child breakpoints.
		if ( $attr && $breakpoint ) {
			$inherited_value = ModuleUtils::use_attr_value(
				[
					'attr'         => $attr,
			...
			$width_value = $width_value ? $width_value : ( $inherited_value['width'] ?? '' );
			$align_self  = $align_self ? $align_self : ( $inherited_value['alignSelf'] ?? '' );
		}
		...
		if ( ! is_string( $align_self ) ) {
			$align_self = '';
		}

		$is_svg = '' !== $src && ImageUtils::is_file_extension( $src, 'svg' );

		if ( $is_svg && ( '' === $width_value || 'auto' === $width_value ) ) {
			// Match D4: empty/auto SVG width forces 100% on the sizing root so the image stays visible.
			$style_declarations->add( 'width', '100%' );
		} elseif ( '' !== $width_value ) {
			$style_declarations->add( 'width', $width_value );
		}

		// Default center on the SVG width target when alignSelf is unset. Left/right alignSelf
		// is emitted by image_sizing_alignment_style_declaration and overrides this when present.
		if ( $is_svg ) {
			if ( '' === $align_self ) {
				$style_declarations->add( 'margin', 'auto' );
			}
		...
		$max_width_value = $attr_value['maxWidth'] ?? null;
		if ( is_string( $max_width_value ) && '' !== $max_width_value ) {
			$style_declarations->add( 'max-width', $max_width_value );
		}

		$min_width_value = $attr_value['minWidth'] ?? null;
		if ( is_string( $min_width_value ) && '' !== $min_width_value ) {
			$style_declarations->add( 'min-width', $min_width_value );
		}
	...
	public static function svg_style_declaration( array $params ): string {
		...
		if ( ! $is_using_icon ) {
			...
			if ( $is_src_svg ) {
				// Child fill: img always fills the SVG sizing root (container or wrap).
				// User/default width lives on `.et_pb_main_blurb_image` via image_width_style_declaration.
				$style_declarations->add( 'width', '100%' );
	...
	public static function module_styles( array $args ): void {
		...
		// Keep width in module + image mode on default sizing; strip maxWidth/minWidth there whenever the custom
		// image-width declaration runs so max/min are emitted only on the image wrapper selector (parity with width).
		...
		$image_sizing_width_attr   = [];
		$image_sizing_max_min_attr = [];
		...
		foreach ( $image_sizing_attr as $breakpoint => $breakpoint_values ) {
			...
			foreach ( $breakpoint_values as $state => $state_value ) {
				...
				$filtered_width_value = array_intersect_key(
					$state_value,
					array_flip(
						[
							'width',
				...
				if ( ! empty( $filtered_width_value ) ) {
					$image_sizing_width_attr[ $breakpoint ][ $state ] = $filtered_width_value;
				}

				$filtered_max_min_value = array_intersect_key(
					$state_value,
					array_flip(
						[
							'maxWidth',
				...
				if ( ! empty( $filtered_max_min_value ) ) {
					$image_sizing_max_min_attr[ $breakpoint ][ $state ] = $filtered_max_min_value;
				}
			}
		}
		...
		// SVG width/default-center needs src (and optional alignSelf) in attrValue so CommonStyle still
		// runs when sizing width is unset. Raster keeps sizing-only attrs.
		$image_src_attr = [];

		foreach ( $attrs['imageIcon']['innerContent'] ?? [] as $breakpoint => $breakpoint_values ) {
			if ( ! is_array( $breakpoint_values ) ) {
				continue;
			}

			foreach ( $breakpoint_values as $state => $state_value ) {
				if ( is_array( $state_value ) && array_key_exists( 'src', $state_value ) ) {
					$image_src_attr[ $breakpoint ][ $state ] = [
						'src' => $state_value['src'],
					];
				}
			}
		...
		$image_width_attr = $is_image_svg
			? array_replace_recursive( [], $image_src_attr, $image_sizing_width_attr )
			: $image_sizing_width_attr;

		// Top-placement SVG uses `.et_pb_main_blurb_image` as the sizing root (not dual container+wrap,
		// which defeats wrap margin:auto and leaves px-width SVGs left-aligned).
		// Relative units (%, em, rem, etc.) must target wrapper only to avoid compounded max-width (#50217).
		// Split per breakpoint/state so a relative child width cannot steal the unset SVG container fallback.
		$image_width_wrap_attr                 = [];
		$image_sizing_alignment_wrap_attr      = [];
		$image_sizing_alignment_container_attr = $image_sizing_attr;

		if ( $is_placement_top && $is_image_svg ) {
			$split_svg_image_width_attrs           = self::_split_svg_image_width_attrs( $image_width_attr );
			$split_svg_image_sizing_alignment      = self::_split_svg_image_sizing_alignment_attrs( $image_sizing_attr );
			$image_width_attr                      = $split_svg_image_width_attrs['container'];
			$image_width_wrap_attr                 = $split_svg_image_width_attrs['wrap'];
			$image_sizing_alignment_container_attr = $split_svg_image_sizing_alignment['container'];
			$image_sizing_alignment_wrap_attr      = $split_svg_image_sizing_alignment['wrap'];
			$image_width_selector                  = $blurb_image_container_selector;
			$image_sizing_alignment_selector       = $blurb_image_container_selector;
		} else {
			...
			$image_width_selector            = $blurb_image_only_mode_selector;
		...
		// SVG width may be split between the container and wrap; max/min always target the wrap.
		$render_image_width_props = $render_image_width_style_declaration ? [
			'selector'            => $image_width_selector,
			'attr'                => $image_width_attr,
			'declarationFunction' => [ self::class, 'image_width_style_declaration' ],
		] : [];
		$render_image_width_wrap_props = ( $render_image_width_style_declaration && ! empty( $image_width_wrap_attr ) ) ? [
			'selector'            => $blurb_image_only_mode_selector,
			'attr'                => $image_width_wrap_attr,
			'declarationFunction' => [ self::class, 'image_width_style_declaration' ],
		] : [];

		$render_image_max_min_width_props = $render_image_width_style_declaration ? [
			'selector'            => $blurb_image_only_mode_selector,
			'attr'                => $image_sizing_max_min_attr,
			'declarationFunction' => [ self::class, 'image_width_style_declaration' ],
		] : [];
		...
		$render_image_sizing_alignment_props = ( $is_placement_top && $render_image_sizing_alignment_style_declaration && ! empty( $image_sizing_alignment_container_attr ) ) ? [
			'selector'            => $image_sizing_alignment_selector,
			'attr'                => $image_sizing_alignment_container_attr,
			'declarationFunction' => [ self::class, 'image_sizing_alignment_style_declaration' ],
		] : [];
		$render_image_sizing_alignment_wrap_props = ( $is_placement_top && $render_image_sizing_alignment_style_declaration && ! empty( $image_sizing_alignment_wrap_attr ) ) ? [
			'selector'            => $blurb_image_only_mode_selector,
			'attr'                => $image_sizing_alignment_wrap_attr,
			'declarationFunction' => [ self::class, 'image_sizing_alignment_style_declaration' ],
		] : [];
					...
					$elements->style(
						[
							...
							'styleProps' => [
								...
								'attrsFilter'    => function ( $element_attrs ) use ( $should_strip_sizing_width_for_default_sizing_style, $render_image_width_style_declaration ) {
									...
									foreach ( $breakpoint_states as $breakpoint => $states ) {
										...
										foreach ( $states as $state ) {
											...
											} elseif ( $render_image_width_style_declaration ) {
												unset( $element_attrs['sizing'][ $breakpoint ][ $state ]['maxWidth'] );
												unset( $element_attrs['sizing'][ $breakpoint ][ $state ]['minWidth'] );
											}
										}
								...
								'advancedStyles' => [
									...
									[
										'componentName' => 'divi/common',
										'props'         => $render_image_width_wrap_props,
									],
									[
										'componentName' => 'divi/common',
										'props'         => $render_image_max_min_width_props,
									],
									[
									...
									[
										'componentName' => 'divi/common',
										'props'         => $render_image_sizing_alignment_wrap_props,
									],
									[
										'componentName' => 'divi/common',
										'props'         => [
