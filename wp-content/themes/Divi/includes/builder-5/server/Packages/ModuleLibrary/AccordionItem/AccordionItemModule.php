...
class AccordionItemModule implements DependencyInterface {
	...
	/**
	 * Determines whether the Accordion Item content is empty.
	 *
	 * @since ??
	 *
	 * @param string $content The Accordion Item content.
	 *
	 * @return bool Whether the content is empty.
	 */
	private static function is_content_empty( string $content ): bool {
		if ( preg_match( '/<(img|video|audio|iframe|object|embed|svg|canvas)(\\s|>|\\/)/i', $content ) ) {
			return false;
		}

		$content_text = trim(
			wp_strip_all_tags(
				// Regex test: https://regex101.com/r/jn4IYn/1.
	...
	public static function render_callback( array $attrs, string $child_modules_content, WP_Block $block, ModuleElements $elements, array $default_printed_style_attrs ): string {
		...
		return Module::render(
			[
				...
				'className'                => HTMLUtility::classnames(
					AccordionItemModuleUtils::get_toggle_class_name( $block->parsed_block['id'], $parent ),
					[
						'et_pb_toggle_empty' => self::is_content_empty( $content ) && empty( $children_ids ) && empty( trim( $child_modules_content ) ),
					]
				),
							...
							'styleProps' => [
								...
								'advancedStyles'           => [
									...
									[
										...
										'props'         => [
											'selector' => ".et_pb_module{$main_class}",
							...
							'styleProps' => [
								'advancedStyles' => [
									...
									[
										...
										'props'         => [
											'selector' => implode(
												',',
												[
													"{$order_class}.et_pb_toggle_close:not(.et_pb_toggle_empty) > .et_pb_toggle_title:before",
													"{$order_class}.et_pb_toggle_close:not(.et_pb_toggle_empty) .et_vb_toggle_overlay",
												]
											),
											'attr'     => $attrs['closedToggleIcon']['decoration']['icon'] ?? [],
									...
									[
										'componentName' => 'divi/common',
										'props'         => [
											'selector' => "{$order_class}.et_pb_toggle_close:not(.et_pb_toggle_empty) .et_vb_toggle_overlay",
											'attr'     => $attrs['closedToggleIcon']['decoration']['icon'] ?? [],
