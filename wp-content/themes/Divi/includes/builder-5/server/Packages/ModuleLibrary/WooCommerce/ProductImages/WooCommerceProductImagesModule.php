...
class WooCommerceProductImagesModule implements DependencyInterface {
	...
	public static function render_callback( array $attrs, string $content, WP_Block $block, ModuleElements $elements ): string {
		...
		$resolved_product_id  = WooCommerceUtils::get_product_id( (string) $product_id );
		...
		$images_html = preg_replace(
			...
			'<div data-product-id="' . esc_attr( $resolved_product_id ) . '" class="woocommerce-product-gallery ',
			$images_html,
			1
		);
