...
class TimelineModule implements DependencyInterface {
	...
	public static function card_style_declaration( array $params ): string {
		...
		if ( $flags['isVerticalAlternating'] ) {
			...
			// Left or right: honor user Card Sizing width/min-width when present. Otherwise unset to
			// clear wider-breakpoint alternating values that only apply to the alternating grid layout.
			$declarations->add( 'min-width', '' !== $card_min_width_raw ? $card_min_width_raw : 'unset' );
			$declarations->add( 'width', '' !== $card_width_raw ? $card_width_raw : 'unset' );
		}
