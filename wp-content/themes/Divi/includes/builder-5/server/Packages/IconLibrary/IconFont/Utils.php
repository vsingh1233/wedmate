...
class Utils {
	...
	/**
	 * Get icon font family for a given icon type.
	 *
	 * Resolves the CSS `font-family` for a font icon `type` via a filterable map.
	 * Defaults: `fa` → `FontAwesome`, `divi` → `ETmodules`.
	 * Unknown / empty types fall back to the `divi` family.
	 *
	 * This function is equivalent of JS function:
	 * {@link /api/js/divi-icon-library/functions/getIconFontFamily getIconFontFamily}
	 * in `@divi/icon-library` package.
	 *
	 * @since ??
	 *
	 * @param string|null $type Icon type value.
	 *
	 * @return string Icon font family name (without quotes).
	 */
	public static function get_icon_font_family( ?string $type ): string {
		static $font_icon_families_cache           = null;
		static $font_icon_families_filters_applied = false;
		...
		// Cached for the request; recomputes when filter presence changes so late.
		// Third-party registration still applies without re-running on every call.
		$filters_exist = false !== has_filter( 'divi_icon_library_font_icon_families' );

		if ( null === $font_icon_families_cache || $filters_exist !== $font_icon_families_filters_applied ) {
			/**
			 * Filters the map of font icon type to CSS font-family.
			 *
			 * @since ??
			 *
			 * @param array $font_icon_families Type → font-family map.
			 *                                  Default `['fa' => 'FontAwesome', 'divi' => 'ETmodules']`.
			 */
			$font_icon_families_cache           = apply_filters( 'divi_icon_library_font_icon_families', $default_families );
			$font_icon_families_filters_applied = $filters_exist;
		}

		$font_icon_families = $font_icon_families_cache;

		if ( null !== $type && isset( $font_icon_families[ $type ] ) ) {
			return $font_icon_families[ $type ];
		}

		return $font_icon_families['divi'] ?? 'ETmodules';
	}
	...
	public static function process_font_icon( $icon, bool $is_font_icons_down = false, bool $is_unicode = false ): ?string {
		...
		static $filtered_icon_symbols = [
			'downIcons' => null,
			'iconList'  => null,
		];
		static $catalog_filters_applied = [
			'downIcons' => false,
			'iconList'  => false,
		];
		static $font_icon_types_cache           = null;
		static $font_icon_types_filters_applied = false;

		$icon_types = [ 'divi', 'fa' ];

		// Cached for the request; recomputes when filter presence changes so late.
		// Third-party registration still applies without re-running on every call.
		$types_filters_exist = false !== has_filter( 'divi_icon_library_font_icon_types' );

		if ( null === $font_icon_types_cache || $types_filters_exist !== $font_icon_types_filters_applied ) {
			/**
			 ...
			 */
			$font_icon_types_cache           = apply_filters( 'divi_icon_library_font_icon_types', $icon_types );
			$font_icon_types_filters_applied = $types_filters_exist;
		}

		$font_icon_types = $font_icon_types_cache;
		...
		if ( ! isset( $icon_symbols[ $icon_json_filename ] ) ) {
			...
			$icon_symbols[ $icon_json_filename ] = json_decode( $wp_filesystem->get_contents( $icon_json_file ), true );
		}

		// Cached for the request; recomputes when filter presence changes so late.
		// Third-party registration still applies without re-filtering on every lookup.
		$filter_name   = $is_font_icons_down ? 'divi_icon_library_font_icon_down_list' : 'divi_icon_library_font_icon_list';
		$filters_exist = false !== has_filter( $filter_name );

		if ( null === $filtered_icon_symbols[ $icon_json_filename ] || $filters_exist !== $catalog_filters_applied[ $icon_json_filename ] ) {
			if ( $is_font_icons_down ) {
				/**
				 * Filters the down-indicator font icon catalog after JSON load (parity with JS `downIcons`).
				 *
				 * @since ??
				 *
				 * @param array $icon_symbols Icon catalog entries (Divi `Icon.Data` shape).
				 */
				$filtered_icon_symbols[ $icon_json_filename ] = apply_filters( 'divi_icon_library_font_icon_down_list', $icon_symbols[ $icon_json_filename ] ?? [] );
			} else {
				/**
				 * Filters the font icon catalog after JSON load (parity with JS `iconList`).
				 *
				 * @since ??
				 *
				 * @param array $icon_symbols Icon catalog entries (Divi `Icon.Data` shape).
				 */
				$filtered_icon_symbols[ $icon_json_filename ] = apply_filters( 'divi_icon_library_font_icon_list', $icon_symbols[ $icon_json_filename ] ?? [] );
			}

			$catalog_filters_applied[ $icon_json_filename ] = $filters_exist;
		}

		$icon_symbols_list = $filtered_icon_symbols[ $icon_json_filename ];
