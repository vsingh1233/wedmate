...
class PostUtility {
	...
	public static function truncate_post( $amount, $echo = true, $post = '', $strip_shortcodes = false, $is_words_length = false ) {
		...
		if ( 'on' === et_get_option( $shortname . '_use_excerpt' ) && ! empty( $post_excerpt ) ) {
			...
			if ( $strip_shortcodes ) {
				...
				$truncate = self::_resolve_dynamic_data_tokens_for_excerpt( $truncate, $post_id );
	...
	private static function _extract_plain_text_from_content( string $content, int $max_length = 0 ): string {
		...
		$extracted_fragments          = [];
		$has_unresolved_dynamic_token = false;
		...
		foreach ( $parsed_blocks as $block ) {
			...
			// Unresolved dynamic tokens inflate the raw fragment length; keep extracting
			// so later blocks that fit after resolution are not dropped from the excerpt.
			if ( ! $has_unresolved_dynamic_token && str_contains( $replacement, '$variable(' ) ) {
				$has_unresolved_dynamic_token = true;
			}
			...
			if ( $length_limit > 0 && ! $has_unresolved_dynamic_token && strlen( implode( ' ', $extracted_fragments ) ) >= $length_limit ) {
				break;
			}
		}
	...
	/**
	 * Resolve D5 dynamic data tokens for lightweight excerpt generation.
	 *
	 * This keeps the lightweight excerpt path fast while still resolving `$variable(...)$`
	 * values by using DynamicData directly. It also restores the previous inner-content
	 * rendering state to prevent leaking global parser context.
	 *
	 * @since ??
	 *
	 * @param string   $content Prepared content.
	 * @param int|null $post_id Current post ID for dynamic data context.
	 *
	 * @return string Content with D5 dynamic data tokens resolved when available.
	 */
	private static function _resolve_dynamic_data_tokens_for_excerpt( string $content, ?int $post_id ): string {
		if ( '' === $content || ! str_contains( $content, '$variable(' ) ) {
			return $content;
		}

		$variable_values = DynamicData::get_variable_values( $content );

		if ( empty( $variable_values ) ) {
			return $content;
		}

		$was_rendering_inner_content = BlockParserStore::is_rendering_inner_content();

		if ( ! $was_rendering_inner_content ) {
			BlockParserStore::set_rendering_inner_content( true );
		}

		try {
			foreach ( array_unique( $variable_values ) as $string_value ) {
				...
				if ( 'content' === $type && is_array( $value ) ) {
					if ( $post_id && ! isset( $value['post_id'] ) ) {
						$value['post_id'] = $post_id;
					}

					$processed_dynamic_content = DynamicContentUtils::get_processed_dynamic_content( $value );
					$replacement              = is_scalar( $processed_dynamic_content ) ? (string) $processed_dynamic_content : '';

					if ( str_contains( $replacement, '$variable(' ) ) {
						$replacement = '';
					}
				}
			...
			return preg_replace( '/\$variable\((.+?)\)\$/', '', $content );
		} finally {
			BlockParserStore::set_rendering_inner_content( $was_rendering_inner_content );
		}
	}
