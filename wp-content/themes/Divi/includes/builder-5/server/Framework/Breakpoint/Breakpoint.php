...
class Breakpoint {
	...
	public static function update( $breakpoints ) {
		$default_items            = self::get_default_settings_values()['items'];
		$validated_existing_items = self::get_settings_values()['items'];
		$existing_items           = is_array( $validated_existing_items )
			? array_replace_recursive( $default_items, $validated_existing_items )
			: $default_items;
		$incoming_items           = isset( $breakpoints['items'] ) && is_array( $breakpoints['items'] )
			? $breakpoints['items']
			: [];

		// Sanitize breakpoints.
		$sanitized_breakpoints = [
			'items' => self::validate_items( array_replace_recursive( $existing_items, $incoming_items ) ),
		];
