...
class RESTRoute {
	...
	public function register_rest_route( string $method, string $route, array $route_args, $route_callback, $permission_callback, string $nonce_policy = self::NONCE_POLICY_ET_AND_WP ): void {
		...
		$registered_methods = array_map( 'trim', explode( ',', strtoupper( $method ) ) );
		foreach ( $registered_methods as $registered_method ) {
			Nonce::add_data( RESTController::get_full_route( $this->_namespace, $full_route ), $registered_method, RESTController::create_nonce( $this->_namespace, $full_route, $registered_method ) );
		}

		add_filter(
			'rest_request_before_callbacks',
			function ( $response, array $handler, WP_REST_Request $request ) use ( $method, $registered_methods, $full_route, $nonce_policy ) {
				if ( null !== $response ) {
				...
				$registered_route = rtrim( RESTController::get_full_route( $this->_namespace, $full_route ), '/' );
				...
				// WordPress dispatches HEAD requests through the matching GET route, so use
				// GET for registered-method matching and the route-specific nonce name too.
				if ( 'HEAD' === $request_method ) {
					$request_method = 'GET';
				}
				if ( ! in_array( $request_method, $registered_methods, true ) ) {
					return $response;
				}
				...
				if ( ! $is_route_match && false !== strpos( $full_route, '?P<' ) ) {
