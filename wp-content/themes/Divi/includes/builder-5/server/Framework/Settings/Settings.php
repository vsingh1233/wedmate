...
class Settings {
	...
	/**
	 * Reset cached page settings values.
	 *
	 * Archive identity depends on the current query, so a singular cache entry must not
	 * leak into a later archive request in the same PHP process (PHPUnit).
	 *
	 * @since ??
	 *
	 * @return void
	...
	public static function reset(): void {
		self::$_page_settings_values     = [];
		self::$_page_settings_is_default = [];
	}

	/**
	 * Archive / non-singular title for post_content page settings.
	 *
	 * WooCommerce shop uses the Shop page title, not the first product in the loop.
	 *
	 * @since ??
	 *
	 * @return string
	 */
	private static function _get_non_singular_page_settings_title() {
		...
		return is_string( $archive_title ) ? $archive_title : '';
	}
	...
	private static function _get_page_settings_values( $post_id ) {
		...
		// Never call get_post( 0 ) — it falls back to the global $post (first loop item on archives).
		$post                 = 0 < $post_id ? get_post( $post_id ) : null;
		$is_tb_layout         = $post && et_theme_builder_is_layout_post_type( $post->post_type );
		$use_archive_identity = ! is_singular() && ! $is_tb_layout;

		// Archive titles depend on the query, not the loop post ID. Reusing a singular cache
		// entry keyed by that ID leaks the previous post's title onto shop/archive TBVB.
		if ( ! $use_archive_identity && ! empty( self::$_page_settings_values[ $post_id ] ) ) {
		...
		$post_thumbnail      = ( $post && ! $use_archive_identity ) ? get_post_thumbnail_id( $post_id ) : '';
		$post_settings_image = $post_thumbnail && '0' !== $post_thumbnail ? $post_thumbnail : '';

		$post_settings_title = $use_archive_identity
			? self::_get_non_singular_page_settings_title()
		...
		$values = [
			...
			'et_pb_post_settings_excerpt'            => ( $post && ! $use_archive_identity ) ? $post->post_excerpt : '',
