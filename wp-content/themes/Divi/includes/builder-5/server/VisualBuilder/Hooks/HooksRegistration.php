...
class HooksRegistration implements DependencyInterface {
	...
	/**
	 * Validate AI agent chat image uploads (JPEG/PNG/WEBP only) via finfo MIME detection.
	 *
	 * @since ??
	 *
	 * @param array  $checked_filetype_and_ext File data array.
	 * @param string $file                     Full path to the file.
	 * @param string $filename                 The name of the file.
	 *
	 * @return array
	 */
	public static function check_filetype_and_ext_ai_agent( array $checked_filetype_and_ext, string $file, string $filename ): array {
		$mimes_ai_agent = [
			'jpg'  => [ 'image/jpeg' ],
			'jpeg' => [ 'image/jpeg' ],
			'jpe'  => [ 'image/jpeg' ],
		...
		if ( file_exists( $file ) && extension_loaded( 'fileinfo' ) ) {
			...
			if ( $ext && $ext !== $filename && isset( $mimes_ai_agent[ $ext ] ) ) {
				$finfo     = finfo_open( FILEINFO_MIME_TYPE );
				$real_mime = finfo_file( $finfo, $file );
				...
				if ( $real_mime && in_array( $real_mime, $mimes_ai_agent[ $ext ], true ) ) {
					return [
						'ext'             => $ext,
						'type'            => $real_mime,
						'proper_filename' => sanitize_file_name( $filename ),
		...
		if ( $ext && $type && isset( $mimes_ai_agent[ $ext ] ) && in_array( $type, $mimes_ai_agent[ $ext ], true ) ) {
			return $checked_filetype_and_ext;
		}
