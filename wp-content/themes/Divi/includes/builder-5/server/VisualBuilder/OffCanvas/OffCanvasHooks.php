...
class OffCanvasHooks {
	...
	public static function save_off_canvas_data( $post_id, $save_context = [] ) {
		...
		foreach ( $deleted_canvas_ids as $deleted_canvas_id ) {
			...
			$is_local_canvas_delete = false;

			// Slot-prefixed runtime IDs (e.g. `header-<uuid>`) must match canonical `_divi_canvas_id`.
			$canonical_deleted_canvas_id = self::_normalize_slot_prefixed_canvas_id( $deleted_canvas_id );
			$layout_slot                 = self::_get_canvas_id_layout_slot( $deleted_canvas_id );
			$is_slot_prefixed            = '' !== $layout_slot;
			$lookup_canvas_id_candidates = array_values(
				array_unique(
					array_filter(
						[
							$deleted_canvas_id,
							$canonical_deleted_canvas_id,
						],
						static function ( $candidate_id ) {
							return is_string( $candidate_id ) && '' !== $candidate_id;
						}
			...
			$canvas_id_meta_query        = [
				'key'     => '_divi_canvas_id',
				'value'   => $lookup_canvas_id_candidates,
				'compare' => 'IN',
			];

			// Prefixed deletes may only target that slot's layout, not every allowed parent.
			$delete_parent_post_ids = self::_get_delete_parent_post_ids_for_layout_slot(
				$layout_slot,
				(int) $post_id,
				$local_parent_post_ids
			);
			...
			if ( ! empty( $delete_parent_post_ids ) ) {
				...
				if ( ! empty( $posts_to_delete ) ) {
					$is_local_canvas_delete = true;
				}
			}

			// Unprefixed IDs may still resolve as context-backed or global canvases.
			if ( ! $is_slot_prefixed && empty( $posts_to_delete ) && ! empty( $local_parent_context_keys ) ) {
				$posts_to_delete = get_posts(
					[
						'post_type'      => self::GLOBAL_CANVAS_POST_TYPE,
						'posts_per_page' => 1,
			...
			if ( ! $is_slot_prefixed && empty( $posts_to_delete ) && $can_manage_global_canvases ) {
				$posts_to_delete = get_posts(
					[
						'post_type'      => self::GLOBAL_CANVAS_POST_TYPE,
						'posts_per_page' => 1,
			...
			if ( true === $is_local_canvas_delete ) {
				$did_delete_local_canvas = true;
			}
		}

		/*
		 * After a successful local-canvas delete, invalidate `_divi_dynamic_assets_canvases_used`,
		 * for the same owner set used when resolving local canvases (saved post + assigned TB layouts).
		 */
		if ( true === $did_delete_local_canvas ) {
			$owner_post_ids_to_invalidate = array_values(
				array_unique(
					array_filter(
						array_map( 'absint', $local_parent_post_ids )
					)
				)
			);

			foreach ( $owner_post_ids_to_invalidate as $owner_post_id ) {
				ET_Core_PageResource::remove_static_resources( $owner_post_id, 'all', true, 'dynamic', true );
				// remove_static_resources() can return before clearing post meta (filesystem/capability).
				ET_Core_PageResource::clear_post_meta_caches( $owner_post_id );
				DynamicAssetsUtils::clear_canvas_ids_cache( $owner_post_id );
			}
		}
	...
	/**
	 * Returns the Theme Builder slot encoded in a runtime canvas ID.
	 *
	 * @since ??
	 *
	 * @param string $canvas_id Raw canvas ID.
	 *
	 * @return string Slot name or empty string when the ID is not slot-prefixed.
	 */
	private static function _get_canvas_id_layout_slot( string $canvas_id ): string {
		$canvas_id = sanitize_text_field( $canvas_id );
		if ( '' === $canvas_id ) {
			return '';
		}
		...
		foreach ( [ 'header', 'body', 'footer' ] as $slot ) {
			if ( str_starts_with( $canvas_id, $slot . '-' ) ) {
				return $slot;
			}
		}
	...
	/**
	 * Resolves the post-backed parent IDs a delete may target for a layout slot.
	 *
	 * Slot-prefixed IDs may only target that slot's layout. Unprefixed IDs keep
	 * the full save allow-list.
	 *
	 * @since ??
	 *
	 * @param string $layout_slot           Slot name or empty string when unprefixed.
	 * @param int    $post_id               Saved post ID.
	 * @param int[]  $local_parent_post_ids Allowed post-backed owners for this save.
	 *
	 * @return int[]
	 */
	private static function _get_delete_parent_post_ids_for_layout_slot(
		string $layout_slot,
		int $post_id,
		array $local_parent_post_ids
	): array {
		if ( '' === $layout_slot ) {
			return $local_parent_post_ids;
		}

		$slot_owner_post_id = self::_get_theme_builder_layout_post_id_by_slot( $post_id, $layout_slot );
		if ( 0 === $slot_owner_post_id ) {
			$layout_post_type_by_slot = [
				'header' => 'et_header_layout',
				'body'   => 'et_body_layout',
				'footer' => 'et_footer_layout',
			];
			$expected_post_type       = $layout_post_type_by_slot[ $layout_slot ] ?? '';
			if ( '' !== $expected_post_type && $expected_post_type === get_post_type( $post_id ) ) {
				$slot_owner_post_id = $post_id;
			}
		}

		if ( 0 === $slot_owner_post_id || ! in_array( $slot_owner_post_id, $local_parent_post_ids, true ) ) {
			return [];
		}

		return [ $slot_owner_post_id ];
	}
	 ...
	 *
	 * A falsy `$post_id` (404 and other post-ID-less requests) skips post-owned canvases
	 * but still expands canvases owned by active Theme Builder layouts.
	 *
	 * @since ??
	 *
	 * @param int    $post_id      Current post ID. `0` on 404 / post-ID-less requests.
	...
	public static function get_all_appended_canvas_content_for_post_and_templates( int $post_id, string $main_content = '' ): string {
		// Post-owned canvases require a truthy owner ID. Theme Builder layout canvases do not.
		$all_canvas_content = $post_id ? self::get_all_appended_canvas_content( $post_id, $main_content ) : '';
