...
class FontsUtility {
	...
	/**
	 * Maximum persisted custom-font family name length after charset strip.
	 *
	 * Used as the `et_uploaded_fonts` option-array key. Agent `fontName` REST
	 * schema uses the same bound.
	 *
	 * @since ??
	 *
	 * @var int
	...
	public static function font_add( array $font_files, string $font_name, array $font_settings, array $overrides = [] ) {
		...
		if ( self::FONT_NAME_MAX_LENGTH < StringUtility::strlen_utf8( $font_name ) ) {
			return new WP_Error(
				'font_name_too_long',
				sprintf(
					/* translators: %d: maximum font name length in characters. */
					esc_html__( 'Font Name Cannot Exceed %d Characters.', 'et_builder_5' ),
					self::FONT_NAME_MAX_LENGTH
				)
			);
		}
