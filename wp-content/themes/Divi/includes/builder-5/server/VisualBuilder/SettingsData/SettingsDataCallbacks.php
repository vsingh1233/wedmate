...
class SettingsDataCallbacks {
	/**
	 * Get Divi AI authentication popup configuration.
	 *
	 * @since ??
	 *
	 * @return array
	 */
	public static function divi_ai_auth(): array {
		return DiviAIAuthService::get_popup_config();
	}
	/**
	...
	private static function _get_current_main_loop_type(): string {
		...
		// WooCommerce shop is a product post type archive (`is_shop()`), even when
		// `is_post_type_archive( 'product' )` is not yet true for the current request.
	...
	private static function _get_non_singular_page_data( string $main_loop_type ): array {
		...
		switch ( $main_loop_type ) {
			...
			case 'post_type_archive':
				...
				// WooCommerce shop archive title is the Shop page title, not the first product
				// in the loop and not the generic "Products" post type label.
				if ( function_exists( 'is_shop' ) && is_shop() && function_exists( 'wc_get_page_id' ) ) {
					$shop_page_id    = wc_get_page_id( 'shop' );
					$shop_page_title = $shop_page_id ? get_the_title( $shop_page_id ) : '';

					if ( is_string( $shop_page_title ) && '' !== $shop_page_title ) {
						$data['title'] = $shop_page_title;
					}
				}

				$queried_object = get_queried_object();
				if ( '' === $data['title'] && $queried_object instanceof WP_Post_Type ) {
	...
	public static function post() {
		...
		if ( null === $return || Conditions::is_test_env() ) {
			...
			// On non-singular pages WordPress sets global $post to the first main-query loop
			// item (e.g. the first product on a shop archive). Using that object as the VB
			// "post" hydrates a singular product layout, leaks the product title, and fires
			// product-only REST modules.
			...
			// Set request_type on Archive pages (including WooCommerce shop).
			...
			$iframe_src = ( isset( $_SERVER['HTTP_HOST'] ) && isset( $_SERVER['REQUEST_URI'] ) ) ?
				( is_ssl() ? 'https://' : 'http://' ) . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) )
				. sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';

			$post_title = $is_singular
				? get_the_title( $post_id )
				: self::_get_non_singular_page_data( self::_get_current_main_loop_type() )['title'];

			$post_url = ( $is_singular && $post_id )
				? get_permalink( $post_id )
				: ( $iframe_src ? esc_url_raw( remove_query_arg( [ 'et_fb', 'app_window', 'PageSpeed' ], $iframe_src ) ) : '' );
			...
			$return = [
				...
				'status'             => $post_status,
				'url'                => $post_url,
				'editUrl'            => ( $is_singular && $post_id ) ? get_edit_post_link( $post_id, 'raw' ) : '',
				'iframeSrc'          => $iframe_src,
				'showPageCreation'   => $post_id ? get_post_meta( $post_id, '_et_pb_show_page_creation', true ) : '',
			];
		}
	...
	public static function settings() {
		...
		if ( null === $return ) {
			...
			$return = [
				...
				'site'         => array_merge(
					[
						'gmtOffsetString' => SiteSettings::get_gmt_offset_string_from_timezone(),
						'url'             => get_site_url(),
					],
					SiteSettings::get_identity_settings()
				),
				'theme'        => [
	 ...
	 *
	 * Returns registered post types keyed by slug with metadata for the Visual Builder and AI agent.
	 *
	 * @since ??
	 */
	public static function post_types() {
		$divi_types = function_exists( 'et_builder_get_builder_post_types' ) ? et_builder_get_builder_post_types() : [];
		$return     = [];

		foreach ( get_post_types( [], 'objects' ) as $slug => $post_type_object ) {
			$return[ $slug ] = [
				'name'              => $slug,
				'label'             => $post_type_object->label,
				'hierarchical'      => (bool) $post_type_object->hierarchical,
				'supportsDivi'      => in_array( $slug, $divi_types, true ),
				'hasArchive'        => (bool) $post_type_object->has_archive,
				'publiclyQueryable' => (bool) $post_type_object->publicly_queryable,
				'builtin'           => (bool) $post_type_object->_builtin,
			];
		}
