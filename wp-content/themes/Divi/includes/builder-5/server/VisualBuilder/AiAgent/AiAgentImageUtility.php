...
class AiAgentImageUtility {
  ...
  public static function verify_chat_ownership( string $chat_id ) {
	...
	/**
	 * Ensure the et-ai-agent base directory exists.
	 *
	 * This folder is public WordPress media: composer thumbnails and LLM encoding
	 * GET `attachment.url` under `uploads/et-ai-agent/{chatId}/`. Divi does not
	 * write `.htaccess` here (`php_flag` 500s Apache+FPM; `FilesMatch`/`Require`
	 * 500s hosts with `AllowOverride FileInfo`). Creates `index.php`
	 * (`Silence is golden.`) when missing. Deletes an existing `.htaccess` if
	 * present; that file was only ever Divi-written (#50337).
	 *
	 * @since ??
	 *
	...
	/**
	 * Upload a JPEG, PNG, or WEBP image to the per-chat folder.
	 *
	 * @since ??
	 *
	...
	public static function import_to_media_library(
		...
		if ( empty( $filetype['type'] ) || ! in_array( $filetype['type'], [ 'image/jpeg', 'image/png', 'image/webp' ], true ) ) {
			return new WP_Error(
				'invalid_mime',
