...
class AiAgentApprovalBinding {
	...
	public static function from_args( string $tool_name, array $args ): ?array {
		switch ( $tool_name ) {
			...
			case 'update_rule':
				return self::_id_binding( self::_read_id_value( self::_get_arg( $args, 'ruleId' ) ) );
			...
			case 'create_menu':
				return self::_create_menu_binding( $args );
	 ...
	 * @return array{binding_type: string, binding_value: string}|null
	 */
	private static function _create_menu_binding( array $args ): ?array {
		return self::_args_hash_binding(
			[
				'name' => self::_read_string( self::_get_arg( $args, 'name' ) ),
			]
		);
	}
