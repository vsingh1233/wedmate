...
class PageManagerController extends RESTController {
	...
	public static function show( WP_REST_Request $request ) {
		...
		// Opt-in full layout for composer context resolve; default remains identity-only.
		if ( rest_sanitize_boolean( $request->get_param( 'include_content' ) ) ) {
	...
	public static function search( WP_REST_Request $request ) {
		...
		$post_types = self::_get_search_post_types( $request );
		...
		$query = self::_query_search_results( array_merge( $query_args, [ 'post_type' => $post_types ] ) );
	...
	/**
	 * Runs a search query and maps its posts into REST rows.
	 *
	 * Authorship is narrowed here rather than by the caller so that each
	 * request restricts posts by the capabilities of the queried types.
	 *
	 * @since ??
	 *
	 * @param array $query_args `WP_Query` arguments, including `post_type`.
	 *
	 * @return array{results: array, total: int, total_pages: int}
	 */
	private static function _query_search_results( array $query_args ): array {
		$post_types = (array) $query_args['post_type'];

		if ( ! self::_can_edit_others_for_post_types( $post_types ) ) {
	 ...
	 *
	 * This function checks if the current user has the permission to use the Visual Builder
	 * and can edit the target post (required for duplicating posts).
	 *
	 * @since ??
	 *
	 * @param WP_REST_Request $request The REST request object.
	 *
	 * @return bool Returns `true` if the current user has the permission to use the Visual Builder and can edit the post, `false` otherwise.
	 */
	public static function duplicate_permission( WP_REST_Request $request ): bool {
	 ...
	 * @param WP_REST_Request $request The REST request object.
	 *
	 * @return bool|\WP_Error Returns `true` if the current user has the permission to use the Visual Builder and can create posts, `false` otherwise.
	 */
	public static function create_permission( WP_REST_Request $request ) {
		$can_create = UserRole::can_current_user_use_visual_builder() && current_user_can( 'edit_posts' );
	 ...
	 * @param WP_REST_Request $request The REST request object.
	 *
	 * @return bool|\WP_Error Returns `true` if the current user has update permissions, `false` otherwise.
	 */
	public static function update_permission( WP_REST_Request $request ) {
		...
		if ( ! UserRole::can_current_user_use_visual_builder() || ! current_user_can( 'edit_posts' ) || 0 >= $post_id ) {
			...
			$can_update = current_user_can( 'edit_post', $post_id );
		}
	 ...
	 * @return bool|\WP_Error Returns `true` if the current user has update permissions, `false` otherwise.
	 */
	public static function update_status_permission( WP_REST_Request $request ) {
		$post_id = absint( $request->get_param( 'id' ) );
	 ...
	 */
	private static function _get_search_post_type( WP_REST_Request $request ): string {
		$post_type = $request->get_param( 'post_type' );
	...
	/**
	 * Resolve searchable post type slugs for a request.
	 *
	 * @since ??
	 *
	 * @param WP_REST_Request $request The REST request object.
	 ...
	 */
	private static function _get_search_post_types( WP_REST_Request $request ): array {
		$post_type        = self::_get_search_post_type( $request );
		$post_type_object = get_post_type_object( $post_type );
	...
	/**
	 * Whether the current user can edit others' posts for all given types.
	 *
	 * @since ??
	 *
	 ...
	 * @return bool
	 */
	private static function _can_edit_others_for_post_types( array $post_types ): bool {
		foreach ( $post_types as $post_type ) {
			$post_type_object = get_post_type_object( $post_type );
	 ...
	 *
	 * This function checks if the current user has the permission to use the Visual Builder
	 * and can edit each post in the reorder list.
	 *
	 * @since ??
	 *
	 * @param WP_REST_Request $request The REST request object.
	 *
	 * @return bool Returns `true` if the current user has the permission to use the Visual Builder and can edit all posts, `false` otherwise.
	 */
	public static function update_order_permission( WP_REST_Request $request ): bool {
		...
		if ( '' === $post_type || ! is_array( $post_ids ) || empty( $post_ids ) ) {
			return false;
		}
		...
		foreach ( array_map( 'absint', $post_ids ) as $post_id ) {
			...
			if ( ! current_user_can( 'edit_post', $post_id ) || get_post_type( $post_id ) !== $post_type ) {
				return false;
			}
		}
	 ...
	 *
	 * This function checks if the current user has the permission to use the Visual Builder
	 * and can edit and delete the target post (required for trashing posts).
	 *
	 * @since ??
	 *
	 * @param WP_REST_Request $request The REST request object.
	 *
	 * @return bool|\WP_Error Returns `true` if the current user has the permission to use the Visual Builder and can trash the post, `false` otherwise.
	 */
	public static function trash_permission( WP_REST_Request $request ) {
		$post_id = absint( $request->get_param( 'id' ) );
		...
		if ( ! UserRole::can_current_user_use_visual_builder() || ! current_user_can( 'edit_posts' ) || 0 >= $post_id ) {
			...
			$can_trash = current_user_can( 'edit_post', $post_id ) && current_user_can( 'delete_post', $post_id );
		}
