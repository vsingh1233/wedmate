...
class ContentMigrationController extends RESTController {
	 ...
	 *
	 * All other content that contains HTML tags is sanitized using `wp_kses_post()` to strip script tags.
	 * Tag-free strings skip `wp_kses_post()` so special characters such as `&` are not encoded as HTML entities.
	 *
	 * @since ??
	 *
	...
	private static function _sanitize_block_attrs_with_allow_html( array $attrs, array $metadata, string $block_name, array $path = [] ): array {
		...
		foreach ( $attrs as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				...
				} elseif ( ! HTMLUtility::contains_html_tags( $value ) ) {
					// Tag-free strings skip wp_kses_post() so special characters such as `&` and `>` are not encoded as HTML entities.
					$sanitized_attrs[ $key ] = et_core_intentionally_unescaped( $value, 'html' );
				} else {
