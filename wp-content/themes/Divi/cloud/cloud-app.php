...
class ET_Cloud_App {
	...
	public static function ajaxRefreshTokens() {
		et_core_security_check( 'edit_posts', 'et_cloud_refresh_token', 'et_cloud_token_nonce' );
	...
	public static function refreshTokens() {
		...
		if ( $is_refresh ) {
			$token_parts   = explode( '.', $refresh_token );
			$token_payload = isset( $token_parts[1] ) ? $token_parts[1] : '';
			// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- JWT payload decode; not used to obfuscate code.
			$refresh_token_data = json_decode( base64_decode( $token_payload ) );
			$endpoint           = ( is_object( $refresh_token_data ) && isset( $refresh_token_data->aud[1] ) ) ? $refresh_token_data->aud[1] : '';
			$url                = self::_get_safe_refresh_url( $endpoint );
	...
	/**
	 * Build a token-refresh URL from an untrusted JWT audience value.
	 *
	 * The JWT audience claim is not signed. Only a same-origin single path segment on
	 * ET_CLOUD_SERVER_URL is accepted, and the outbound URL is rebuilt from
	 * the constant plus that segment.
	 *
	 * @param mixed $endpoint JWT aud[1] value.
	 *
	 * @return string|false Refresh URL, or false when the endpoint is not allowed.
	 */
	private static function _get_safe_refresh_url( $endpoint ) {
		if ( ! is_string( $endpoint ) || '' === $endpoint ) {
			return false;
		}

		$allowed = wp_parse_url( ET_CLOUD_SERVER_URL );
		$parsed  = wp_parse_url( $endpoint );

		if ( ! is_array( $allowed ) || ! is_array( $parsed ) ) {
			return false;
		}

		if ( empty( $allowed['scheme'] ) || empty( $allowed['host'] ) || empty( $parsed['scheme'] ) || empty( $parsed['host'] ) ) {
			return false;
		}
		...
		if ( strtolower( $allowed['scheme'] ) !== strtolower( $parsed['scheme'] ) ) {
			return false;
		}

		if ( strtolower( $allowed['host'] ) !== strtolower( $parsed['host'] ) ) {
			return false;
		}

		$default_port = ( 'https' === strtolower( $allowed['scheme'] ) ) ? 443 : 80;
		$allowed_port = isset( $allowed['port'] ) ? (int) $allowed['port'] : $default_port;
		$parsed_port  = isset( $parsed['port'] ) ? (int) $parsed['port'] : $default_port;

		if ( $allowed_port !== $parsed_port ) {
			return false;
		}
		...
		if ( 1 !== preg_match( '/^[A-Za-z0-9_-]{8,64}$/', $path ) ) {
			return false;
		}

		return untrailingslashit( ET_CLOUD_SERVER_URL ) . '/' . $path . '/wp-json/auth/v1/token';
	}
